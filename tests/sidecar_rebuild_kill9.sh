...
ENGINE_DIR="$ROOT_DIR/fuju-trace-engine"
BIN="$ENGINE_DIR/target/release/examples/scale_bench"
DATA_DIR="$(mktemp -d)"
LOG_DIR="$DATA_DIR/logs"
SPANS="${FUJU_TRACE_SIDECAR_CRASH_SPANS:-2000}"
PID=""
...
cargo build --offline --manifest-path "$ENGINE_DIR/Cargo.toml" \
  -p fuju-trace-engine --example scale_bench --release --features test-failpoints
...
crash_and_recover() {
  ...
  FUJU_TRACE_TEST_SIDECAR_BEFORE_RENAME="$kind" \
    FUJU_TRACE_TEST_SIDECAR_MARKER="$marker" \
