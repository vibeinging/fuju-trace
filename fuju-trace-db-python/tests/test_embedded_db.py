...
SDK_PYTHON_DIR = Path(__file__).resolve().parents[2] / "fuju-trace-sdk" / "python"
...
def _python_multiworker_write(data_dir: str, worker_id: int, count: int, queue) -> None:
    try:
        with FujuTraceDB.open(data_dir, tenant_id=1) as db:
...
def test_python_embedded_db_ingests_searches_and_reads_span_detail():
    with tempfile.TemporaryDirectory() as tmp:
        with FujuTraceDB.open(tmp, tenant_id=1) as db:
        ...
        with FujuTraceDB.open(tmp, tenant_id=1) as reopened:
...
def test_python_sdk_server_embedded_exporters_write_real_db():
    from fuju_trace import BufferedDbExporter, SpoolConsumer, SpoolDbExporter, Tracer, init_fuju_trace
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        runtime.close()
        with FujuTraceDB.open(helper_data_dir, tenant_id=1) as db:
            hits = db.search({"text": "盗刷", "k": 10})
            helper_messages = "\n".join(
                message
                for hit in hits
...
def test_python_embedded_db_allows_multiple_handles_with_serialized_writes():
    with tempfile.TemporaryDirectory() as tmp:
        first = FujuTraceDB.open(tmp, tenant_id=1)
        second = FujuTraceDB.open(tmp, tenant_id=1)
        try:
            first.ingest([
                {
        ...
        reopened = FujuTraceDB.open(tmp, tenant_id=1)
        try:
...
def test_python_embedded_db_general_route_json_and_closed_errors():
    with tempfile.TemporaryDirectory() as tmp:
        db = FujuTraceDB.open({"dataDir": tmp, "tenantId": 1})
        try:
...
def test_python_fastapi_router_reuses_embedded_db_route_boundary():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        with FujuTraceDB.open(tmp) as db:
            app = fastapi.FastAPI()
            ...
            ingest = client.post("/fuju_trace/v1/ingest", json=builder.events(), headers={"X-Tenant-Id": "42"})
            assert ingest.status_code == 200
            assert ingest.json()["ingested"] == 3

            search = client.post(
                "/fuju_trace/v1/search",
...
def test_python_fastapi_app_uses_lifespan_for_owned_db():
    ...
    from fuju_trace_db.fastapi import create_fuju_trace_app

    with tempfile.TemporaryDirectory() as tmp:
        app = create_fuju_trace_app(tmp, tenant_id=42)
...
def test_python_cli_rejects_multi_worker_embedded_serve():
    from fuju_trace_db.cli import _parse_bind, main
