"""Optional FastAPI adapter for fuju-trace-db."""
...
def _require_fastapi() -> None:
    if APIRouter is None or FastAPI is None or HTTPException is None or JSONResponse is None:
        raise RuntimeError("FastAPI support requires: pip install 'fuju-trace-db[server]'")
...
def create_fuju_trace_router(
    db: FujuTraceDB,
    *,
    tenant_header: str = "X-Tenant-Id",
    default_tenant_id: str | int | None = None,
):
    """Expose a FujuTraceDB handle through FastAPI.
...
def create_fuju_trace_app(
    db_or_data_dir: FujuTraceDB | str | Path,
    *,
    tenant_id: str | int | None = None,
    tenant_header: str = "X-Tenant-Id",
):
    """Create a small FastAPI app around an embedded Fuju Trace DB."""

    _require_fastapi()
    owns_db = not isinstance(db_or_data_dir, FujuTraceDB)
    db = db_or_data_dir if isinstance(db_or_data_dir, FujuTraceDB) else FujuTraceDB.open(db_or_data_dir, tenant_id=tenant_id)
    if owns_db:
        @asynccontextmanager
        ...
        app = FastAPI(title="Fuju Trace", lifespan=lifespan)
    else:
        app = FastAPI(title="Fuju Trace")
    app.include_router(create_fuju_trace_router(db, tenant_header=tenant_header, default_tenant_id=tenant_id))
    return app


__all__ = ["create_fuju_trace_app", "create_fuju_trace_router"]
