"""Command line entrypoint for fuju-trace-db."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="fuju-trace-db")
    sub = parser.add_subparsers(dest="command", required=True)
    serve = sub.add_parser("serve", help="serve an embedded Fuju Trace data dir over HTTP")
    serve.add_argument("--data-dir", required=True, help="Fuju Trace data directory")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "serve":
        if args.workers != 1:
            parser.error("embedded serve supports exactly one worker; use a single Fuju Trace server for multi-worker apps")
        ...
        try:
            ...
            raise SystemExit("serve requires: pip install 'fuju-trace-db[server]'") from err
        from .fastapi import create_fuju_trace_app

        app = create_fuju_trace_app(args.data_dir, tenant_id=args.tenant_id)
