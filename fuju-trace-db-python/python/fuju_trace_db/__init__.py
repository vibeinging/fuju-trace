"""Embedded Fuju Trace DB for Python.
...
from ._native import NativeFujuTraceDB
...
class SpanEventBuilder:
    """Build Fuju Trace wire events without exposing seq/event_type details."""
    ...
    def ingest(self, db: "FujuTraceDB", **options: Any) -> Any:
...
class FujuTraceDB:
    """Embedded Fuju Trace database handle."""

    def __init__(self, native: NativeFujuTraceDB, *, tenant_id: TenantId | None = None) -> None:
        self._native = native
    ...
    def open(
        ...
        **options: Any,
        ...
        if not data_dir:
            raise ValueError("FujuTraceDB.open requires a data_dir")
        if "readOnly" in options or "read_only" in options:
            raise ValueError("OpenOptions.readOnly is not supported yet")
        return cls(NativeFujuTraceDB(str(data_dir)), tenant_id=tenant_id)

    def __enter__(self) -> "FujuTraceDB":
        self._ensure_open()
        return self
    ...
    def _ensure_open(self) -> None:
        if self._closed:
            raise RuntimeError("FujuTraceDB is closed")
