...
#[pyclass(name = "NativeFujuTraceDB")]
...
impl NativeFujuTraceDb {
            ...
            let coord = WriteCoordinator::open_durable(&dir)
                .map_err(|e| {
                    py_runtime_err(format!(
                        "open Fuju Trace data dir failed for data_dir={}: {e}. If startup or writes are slow, check db.lock_metrics() or FujuTraceRuntime.health()['lock']; lock timeouts include the owner process.",
                        dir.display()
                    ))
                })?;
            coord.recover();
    ...
    pub fn route_json(
        ...
        if (200..300).contains(&status) {
            ...
            Err(py_runtime_err(format!(
                "Fuju Trace request failed: status={status} body={response}"
            )))
        }
    }
    ...
    fn ensure_open(&self) -> PyResult<()> {
        if self.closed {
            Err(py_runtime_err("FujuTraceDB is closed"))
        } else {
            Ok(())
        }
    }
