//! fuju-trace-vecindex-graph —— 团队 **graph_index**（图式向量 ANN）接入引擎的 [`fuju_trace_engine::GraphIndex`] 接缝。
//!
...
//! （不是搜完再筛——那样选择性谓词下召回会崩，见 `fuju-trace-engine/graph.rs` 的实证对照）。本 crate 把引擎给的
//! `&dyn Fn(u64,u64)->bool` 闭包包成 `(filter_ctx, filter_fn)` 一对传进 C，C 遍历时回调它。见 `ABI.md`。
//!
//! ```ignore
//! use fuju_trace_engine::CoordinatorBuilder;
//! use fuju_trace_vecindex_graph::GraphAnnFfi;
...
use fuju_trace_engine::GraphIndex;
...
mod mock {
    ...
    unsafe extern "C" fn vexgraph_open(dim: u32, _m: u32, _ef: u32) -> *mut c_void {
        Box::into_raw(Box::new(MockGraph {
            dim,
            pts: Vec::new(),
        })) as *mut c_void
    }
    ...
    #[no_mangle]
    unsafe extern "C" fn vexgraph_add(
        handle: *mut c_void,
        trace_id: u64,
        span_id: u64,
        vec: *const f32,
...
mod tests {
    ...
    fn dim_mismatch_and_empty_are_safe() {
        ...
        assert!(
            idx.search(&[1.0, 2.0, 3.0, 4.0], 3, &|_, _| true)
                .is_empty(),
            "没有点"
        );
