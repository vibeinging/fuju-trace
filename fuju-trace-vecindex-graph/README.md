# fuju-trace-vecindex-graph

Fuju Trace 的 **graph_index FFI 向量索引** —— 通过 C ABI 接外部 graph_index 库,实现引擎的 `GraphIndex` trait。

> 状态:**FFI 接缝就绪**(含跨边界进图过滤回调),默认离线 mock。需要复用外部 graph_index 时，用 `--features link` 链接真实库。

## 为什么有这个

引擎默认向量索引是**自研磁盘型多层 HNSW**(`fuju-trace-engine` 的 `DiskGraphIndex`,落盘 + 缓冲池 + 进图过滤)——开箱即用、零依赖。这个 crate 提供**接外部 graph_index** 的可选路径，用于客户已有库、对标或特殊索引场景。
...
| 向量索引 | 在哪 | 何时用 |
|---|---|---|
| `DiskGraphIndex`(自研 HNSW) | `fuju-trace-engine` | 默认,零依赖,落盘 |
