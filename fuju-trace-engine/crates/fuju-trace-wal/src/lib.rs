...
use fuju_trace_core::fold::{FoldInput, SpanFields};
use fuju_trace_core::ids::WalLsn;
...
pub struct Wal {
    next_lsn: u64,
    backing: Backing,
    /// A failed append can leave a torn tail. Refuse later acks until reopen repairs it.
    failed: bool,
}
...
impl Wal {
    ...
    pub fn open(path: impl AsRef<Path>) -> std::io::Result<Self> {
        ...
        let checkpoint = load_wal_state(&state_path)
            ...
            .unwrap_or_default();
        let existing = match read_file_from(&path, checkpoint.scanned_len) {
            Ok(existing) => existing,
            Err(error) if error.kind() == std::io::ErrorKind::NotFound && file_len == 0 => {
                Vec::new()
            }
    ...
    /// Recoverable append path for API callers. Never advances LSN on I/O failure.
    pub fn try_append_committed(&mut self, records: &[WalRecord]) -> std::io::Result<WalLsn> {
        if self.failed {
            return Err(std::io::Error::new(
                std::io::ErrorKind::Other,
                "WAL append failed earlier; reopen the data dir before writing again",
            ));
        }
        ...
        match &mut self.backing {
            ...
            Backing::File {
                ...
                if let Err(error) = file.write_all(&frame).and_then(|_| file.sync_data()) {
                    self.failed = true;
                    return Err(error);
                }
    ...
    pub fn refresh_from_disk_after(&mut self, from: WalLsn) -> (WalLsn, Vec<(u64, WalRecord)>) {
        ...
        } = &mut self.backing
        {
            ...
            if file_len < *scanned_len {
                ...
                let (frames, consumed) = parse_frames_with_consumed(&existing);
                self.failed = true; // an acknowledged prefix disappeared; only reopen can repair state
            ...
            let (frames, consumed) = parse_frames_with_consumed(&existing);
            if consumed < existing.len() {
                self.failed = true; // never append an ack behind a torn tail
            }
...
mod tests {
    ...
    #[test]
    fn failed_append_does_not_ack_or_allow_another_write() {
        let path = temp_path();
        let mut wal = Wal::open(&path).unwrap();
        if let Backing::File { file, .. } = &mut wal.backing {
            *file = File::open(&path).unwrap(); // force a write error without depending on disk space
        }
        assert!(wal.try_append_committed(&[rec("failed", 1)]).is_err());
        assert_eq!(wal.committed_tail().get(), 0);
        assert!(wal.try_append_committed(&[rec("retry", 2)]).is_err());
        drop(wal);

        let mut reopened = Wal::open(&path).unwrap();
        reopened.try_append_committed(&[rec("ok", 3)]).unwrap();
        assert_eq!(reopened.replay_after(WalLsn::new(0)).len(), 1);
        let _ = std::fs::remove_file(&path);
    }
    ...
    fn torn_tail_is_dropped() {
        ...
        wal.try_append_committed(&[rec("after-repair", 3)]).unwrap();
        assert_eq!(wal.replay_after(WalLsn::new(0)).len(), 2);
        let _ = std::fs::remove_file(&path);
    }
}
