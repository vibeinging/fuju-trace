impl WriteCoordinator {
    ...
    fn alloc_chunk_id(&self) -> fuju_trace_core::ids::ChunkId {
        let mut g = self.next_chunk_id.lock().unwrap();
        let id = *g;
        *g += 1;
        fuju_trace_core::ids::ChunkId::new(id)
    }
    ...
    pub fn commit_upgrade(
        ...
        trace_id: u64,
        span_id: u64,
        fields: fuju_trace_core::fold::SpanFields,
    ) {
        let _process = self.acquire_process_lock("write");
        let _w = self.write_lock.lock().unwrap();
        self.refresh_from_disk_locked();
    ...
    pub fn metrics(&self) -> String {
        ...
        out.push_str("# HELP fuju_trace_manifest_version Manifest 版本号（每次 commit +1）。\n");
        out.push_str("# TYPE fuju_trace_manifest_version gauge\n");
        out.push_str(&format!("fuju_trace_manifest_version {version}\n\n"));

        out.push_str(
            "# HELP fuju_trace_format_version 数据格式版本（persist::FORMAT_VER，升级迁移用）。\n",
        );
        out.push_str("# TYPE fuju_trace_format_version gauge\n");
        out.push_str(&format!("fuju_trace_format_version {}\n\n", persist::FORMAT_VER));

        out.push_str("# HELP fuju_trace_segments_live 活跃段数（含 sealed/live/compacting）。\n");
        out.push_str("# TYPE fuju_trace_segments_live gauge\n");
        out.push_str(&format!("fuju_trace_segments_live {segments}\n\n"));

        out.push_str("# HELP fuju_trace_memtable_rows 活内存表行数。\n");
        out.push_str("# TYPE fuju_trace_memtable_rows gauge\n");
        out.push_str(&format!("fuju_trace_memtable_rows {memtable_rows}\n\n"));

        out.push_str(
            "# HELP fuju_trace_segments_dead 待回收 dead 段数（compaction 摘下、等水位满足删）。\n",
        );
        out.push_str("# TYPE fuju_trace_segments_dead gauge\n");
        out.push_str(&format!("fuju_trace_segments_dead {dead}\n\n"));

        out.push_str("# HELP fuju_trace_readers_active 活跃快照读者数（pin 了某版本的）。\n");
        out.push_str("# TYPE fuju_trace_readers_active gauge\n");
        out.push_str(&format!("fuju_trace_readers_active {active_readers}\n\n"));

        out.push_str("# HELP fuju_trace_wal_committed_tail 已确认的最大 WAL LSN。\n");
        out.push_str("# TYPE fuju_trace_wal_committed_tail counter\n");
        out.push_str(&format!("fuju_trace_wal_committed_tail {committed_tail}\n\n"));

        out.push_str("# HELP fuju_trace_flush_threshold 内存表自动刷盘阈值（行数）。\n");
        out.push_str("# TYPE fuju_trace_flush_threshold gauge\n");
        out.push_str(&format!("fuju_trace_flush_threshold {flush_threshold}\n\n"));

        out.push_str("# HELP fuju_trace_read_model_rollup_ready rollup 是否已加载到当前进程。\n");
        out.push_str("# TYPE fuju_trace_read_model_rollup_ready gauge\n");
        out.push_str(&format!(
            "fuju_trace_read_model_rollup_ready {}\n\n",
            u8::from(read_model_state.rollup_ready)
        ));
        out.push_str("# HELP fuju_trace_read_model_filter_ready attrs 过滤索引是否已加载到当前进程。\n");
        out.push_str("# TYPE fuju_trace_read_model_filter_ready gauge\n");
        out.push_str(&format!(
            "fuju_trace_read_model_filter_ready {}\n\n",
            u8::from(read_model_state.filter_attrs_ready)
        ));
        out.push_str("# HELP fuju_trace_read_model_search_ready BM25 和 bloom 是否已加载到当前进程。\n");
        out.push_str("# TYPE fuju_trace_read_model_search_ready gauge\n");
        out.push_str(&format!(
            "fuju_trace_read_model_search_ready {}\n\n",
            u8::from(search_read_model_ready)
        ));

        out.push_str("# HELP fuju_trace_filter_attrs 检索过滤属性边车条目数。\n");
        out.push_str("# TYPE fuju_trace_filter_attrs gauge\n");
        out.push_str(&format!("fuju_trace_filter_attrs {filter_attrs}\n\n"));
        out.push_str("# HELP fuju_trace_filter_attr_postings 检索过滤属性 postings 条目数。\n");
        out.push_str("# TYPE fuju_trace_filter_attr_postings gauge\n");
        out.push_str(&format!(
            "fuju_trace_filter_attr_postings {filter_attr_postings}\n\n"
        ));
        out.push_str(
            "# HELP fuju_trace_filter_attr_disabled_postings 被预算禁用的过滤属性 postings 数。\n",
        );
        out.push_str("# TYPE fuju_trace_filter_attr_disabled_postings gauge\n");
        out.push_str(&format!(
            "fuju_trace_filter_attr_disabled_postings {filter_attr_disabled_postings}\n\n"
        ));

        out.push_str("# HELP fuju_trace_fold_cache_entries 段折叠缓存条目数（解码后的段）。\n");
        out.push_str("# TYPE fuju_trace_fold_cache_entries gauge\n");
        out.push_str(&format!("fuju_trace_fold_cache_entries {fold_cache_entries}\n\n"));

        out.push_str("# HELP fuju_trace_seg_bloom_count 段级 key Bloom 条目数。\n");
        out.push_str("# TYPE fuju_trace_seg_bloom_count gauge\n");
        out.push_str(&format!("fuju_trace_seg_bloom_count {bloom_count}\n\n"));

        if let Some(lock) = process_lock_metrics {
            out.push_str("# HELP fuju_trace_process_lock_acquire_total embedded 进程锁 acquire 次数。\n");
            out.push_str("# TYPE fuju_trace_process_lock_acquire_total counter\n");
            out.push_str(&format!(
                "fuju_trace_process_lock_acquire_total {}\n\n",
                lock.acquire_count
            ));

            out.push_str("# HELP fuju_trace_process_lock_try_acquire_total embedded 进程锁 try_acquire 次数。\n");
            out.push_str("# TYPE fuju_trace_process_lock_try_acquire_total counter\n");
            out.push_str(&format!(
                "fuju_trace_process_lock_try_acquire_total {}\n\n",
                lock.try_acquire_count
            ));

            out.push_str("# HELP fuju_trace_process_lock_wait_total embedded 进程锁发生等待的次数。\n");
            out.push_str("# TYPE fuju_trace_process_lock_wait_total counter\n");
            out.push_str(&format!("fuju_trace_process_lock_wait_total {}\n\n", lock.wait_count));

            out.push_str("# HELP fuju_trace_process_lock_active_waiters 当前正在等 embedded 进程锁的线程数。\n");
            out.push_str("# TYPE fuju_trace_process_lock_active_waiters gauge\n");
            out.push_str(&format!(
                "fuju_trace_process_lock_active_waiters {}\n\n",
                lock.active_wait_count
            ));

            out.push_str("# HELP fuju_trace_process_lock_wait_seconds_total embedded 进程锁累计等待秒数。\n");
            out.push_str("# TYPE fuju_trace_process_lock_wait_seconds_total counter\n");
            out.push_str(&format!(
                "fuju_trace_process_lock_wait_seconds_total {}\n\n",
                (lock.wait_ns as f64) / 1_000_000_000.0
            ));

            out.push_str("# HELP fuju_trace_process_lock_timeout_total embedded 进程锁等待超时次数。\n");
            out.push_str("# TYPE fuju_trace_process_lock_timeout_total counter\n");
            out.push_str(&format!(
                "fuju_trace_process_lock_timeout_total {}\n\n",
                lock.timeout_count
            ));

            out.push_str("# HELP fuju_trace_process_lock_try_busy_total try_acquire 发现锁正忙的次数。\n");
            out.push_str("# TYPE fuju_trace_process_lock_try_busy_total counter\n");
            out.push_str(&format!(
                "fuju_trace_process_lock_try_busy_total {}\n\n",
                lock.try_busy_count
            ));

            out.push_str("# HELP fuju_trace_process_lock_stale_cleared_total 清掉 stale 进程锁的次数。\n");
            out.push_str("# TYPE fuju_trace_process_lock_stale_cleared_total counter\n");
            out.push_str(&format!(
                "fuju_trace_process_lock_stale_cleared_total {}\n\n",
                lock.stale_lock_cleared_count
            ));

            out.push_str("# HELP fuju_trace_process_reader_pin_total 跨进程 reader pin 创建次数。\n");
            out.push_str("# TYPE fuju_trace_process_reader_pin_total counter\n");
            out.push_str(&format!(
                "fuju_trace_process_reader_pin_total {}\n\n",
                lock.reader_pin_count
            ));

            out.push_str("# HELP fuju_trace_process_reader_stale_cleared_total 清掉 stale reader pin 的次数。\n");
            out.push_str("# TYPE fuju_trace_process_reader_stale_cleared_total counter\n");
            out.push_str(&format!(
                "fuju_trace_process_reader_stale_cleared_total {}\n\n",
                lock.stale_reader_cleared_count
            ));
        }

        out.push_str("# HELP fuju_trace_datasets 评测数据集数。\n");
        out.push_str("# TYPE fuju_trace_datasets gauge\n");
        out.push_str(&format!("fuju_trace_datasets {datasets}\n"));
