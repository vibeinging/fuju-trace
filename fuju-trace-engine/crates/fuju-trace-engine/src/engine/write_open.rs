impl WriteCoordinator {
    ...
    fn acquire_process_lock(&self, name: &str) -> Option<process_lock::ProcessLockGuard> {
        self.process_lock.as_ref().map(|mgr| {
            mgr.acquire(name)
                .unwrap_or_else(|e| panic!("Fuju Trace {name} process lock failed: {e}"))
        })
    }
