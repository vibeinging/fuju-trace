...
fn bm25_filter_set_budget_bytes() -> usize {
    std::env::var("FUJU_TRACE_BM25_FILTER_SET_BUDGET_BYTES")
        .ok()
...
impl WriteCoordinator {
    ...
    /// 默认磁盘图直接保存向量；注入自定义图时先写独立向量文件，供重启重建。
    pub fn index_embedding(&self, trace_id: u64, span_id: u64, embedding: Vec<f32>) {
        self.try_index_embedding(trace_id, span_id, embedding)
            .expect("Fuju Trace vector index insert failed");
    }

    pub fn try_index_embedding(
        &self,
        trace_id: u64,
        span_id: u64,
        embedding: Vec<f32>,
    ) -> std::io::Result<()> {
        self.try_index_embeddings(vec![(trace_id, span_id, embedding)])
    }

    /// 批量建图：只取一次跨进程锁、刷新一次，并对旧式向量文件只同步一次。
    pub fn try_index_embeddings(
        &self,
        items: Vec<(u64, u64, Vec<f32>)>,
    ) -> std::io::Result<()> {
        if items.is_empty() {
            return Ok(());
        }
        ...
        if let Some(p) = &self.vector_path {
            vecstore::append_batch(p, &items)?;
        }
        for (trace_id, span_id, embedding) in items {
            self.graph
                .try_index_embedding(trace_id, span_id, embedding)?;
        }
        Ok(())
    }
