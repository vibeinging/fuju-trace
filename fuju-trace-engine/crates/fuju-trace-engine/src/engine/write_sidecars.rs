...
impl WriteCoordinator {
    pub fn trace_aggregate_rollup_spans(
        ...
        self.trace_rollup_spans_inner(q, filter, true)
    }

    pub(crate) fn trace_rollup_spans_unordered(
        &self,
        q: &TraceQuery,
        filter: &SearchFilter,
    ) -> Option<(Vec<FoldedSpan>, ReadPlanStats)> {
        self.trace_rollup_spans_inner(q, filter, false)
    }

    fn trace_rollup_spans_inner(
        &self,
        q: &TraceQuery,
        filter: &SearchFilter,
        ordered: bool,
        ...
        let result = if ordered {
            self.trace_rollup.lock().unwrap().query(q, filter)?
        } else {
            self.trace_rollup.lock().unwrap().query_unordered(q, filter)?
        };
        if result
            .1
            .fallback_reason
    ...
    pub(crate) fn trace_rollup_page(
        &self,
        q: &TraceQuery,
        filter: &SearchFilter,
        page_filter: &crate::trace_rollup::RollupPageFilter,
        sort_by: &str,
        cursor: usize,
        limit: usize,
        ...
        self.trace_rollup
            .lock()
            .unwrap()
            .query_page(q, filter, page_filter, sort_by, cursor, limit)
    }
    ...
    fn persist_read_model_sidecars(&self) {
        // 四份派生缓存使用独立文件和锁；并行保存缩短显式 flush 的等待时间。
        // 调用方仍持有写锁，manifest 版本和水位在整个保存期间不变。
        std::thread::scope(|scope| {
            scope.spawn(|| self.persist_trace_rollup_segments());
            scope.spawn(|| self.persist_filter_attrs_segments());
            scope.spawn(|| self.persist_bm25_segments());
            scope.spawn(|| self.persist_seg_key_bloom_segments());
        });
    }
...
fn save_seg_key_bloom_cache(
    ...
    let checksum = fuju_trace_wal::crc32(&out);
...
fn load_seg_key_bloom_cache(
    ...
    if fuju_trace_wal::crc32(payload) != expected_checksum {
        return None;
    }
