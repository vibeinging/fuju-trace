impl WriteCoordinator {
    ...
    fn index_record_inner(&self, r: &WalRecord, update_rollup: bool, update_filter_attrs: bool) {
        ...
        Self::apply_session_record(&mut idx, r);
    }

    /// Batch writes take each sidecar lock once; recovery keeps the single-record path above.
    fn index_records_batch(&self, records: &[WalRecord]) {
        for record in records {
            self.index_record_text(record);
        }
        {
            let mut attrs = self.filter_attrs.lock().unwrap();
            for record in records {
                attrs.apply_record(record);
            }
        }
        {
            let mut rollup = self.trace_rollup.lock().unwrap();
            for record in records {
                rollup.apply_record(record);
            }
        }
        {
            let mut sessions = self.session_idx.lock().unwrap();
            for record in records {
                Self::apply_session_record(&mut sessions, record);
            }
        }
    }

    fn index_record_text(&self, r: &WalRecord) {
    ...
    fn apply_session_record(idx: &mut SessionIndex, r: &WalRecord) {
        ...
        match r.identity.event_type {
            fuju_trace_core::event::EventType::SpanStart => new.has_start = true,
            fuju_trace_core::event::EventType::SpanEnd => new.has_end = true,
            _ => {}
        }
    ...
    pub fn ingest(&self, records: Vec<WalRecord>) -> WalLsn {
        self.try_ingest(records)
            .expect("Fuju Trace WAL append or sync failed")
    }

    /// 成功同步 WAL 后才更新可查询索引；失败时不确认、也不发布这一批数据。
    pub fn try_ingest(&self, records: Vec<WalRecord>) -> std::io::Result<WalLsn> {
        ...
        let last = wal.try_append_committed(&records)?;
        drop(wal);
        self.index_records_batch(&records);
        {
    ...
    pub fn ingest_wire_for_tenant(
        ...
        self.try_ingest_wire_for_tenant(records, tenant)
            .expect("Fuju Trace WAL append or sync failed")
    }
