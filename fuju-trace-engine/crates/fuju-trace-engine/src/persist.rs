...
//! 重放的部分）就丢了。
//!
//! 落盘：`[crc32 u32][payload]`，原子写（tmp + fsync + rename）。SpanFields 复用 `fuju_trace_wal::encode_span_fields`，
...
use fuju_trace_core::ids::{ChunkId, ManifestVersion, SegmentId, WalLsn};
        ...
        match &e.upgrade_ref {
            ...
            Some(up) => {
                ...
                for (&(t, s), f) in patches {
                    put_u64(&mut b, t);
                    put_u64(&mut b, s);
                    put_blob(&mut b, &fuju_trace_wal::encode_span_fields(f));
                }
            }
        }
    }
        ...
        let upgrade_ref = match c.u8()? {
            ...
            _ => {
                ...
                for _ in 0..patch_n {
                    ...
                    let s = c.u64()?;
                    let blob = c.blob()?;
                    patches.insert((t, s), fuju_trace_wal::decode_span_fields(blob)?);
                }
