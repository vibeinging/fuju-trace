...
use crate::{
    parse_otlp_traces, parse_wire_batch, AnnotationStatus, AnnotationTarget, DatasetAssociation,
...
impl EngineJsonApi {
    ...
    pub fn route_with_tenant(
        ...
        match (method, base) {
            ...
            ("POST", "/v1/traces") => match parse_otlp_traces(body) {
                Ok(recs) => match self.coord.try_ingest_wire_for_tenant(recs, tenant) {
            ...
            // 控制台数据端点（前端 fuju-trace-console 对接）：会话游标分页 / 轮次 / trace span / span 详情。
