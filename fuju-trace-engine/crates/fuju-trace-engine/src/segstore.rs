...
//!
//! 编码复用 WAL 同一套（`fuju_trace_wal::encode_records`/`decode_records`），不再各写一份记录序列化。
...
use fuju_trace_core::fold::FoldInput;
use fuju_trace_core::ids::SegmentId;
use fuju_trace_wal::{Crc32, WalRecord};
...
impl FileSegmentStore {
    ...
    fn point_records(&self, seg: SegmentId, keys: &HashSet<(u64, u64)>) -> Option<KeyedRecordScan> {
        ...
        for entry in entries {
            ...
            if fuju_trace_wal::crc32(&bytes) != entry.record_crc {
                return None;
            }
            let record = fuju_trace_wal::decode_record(&bytes)?;
    ...
    fn full_record_scan(&self, seg: SegmentId) -> FullRecordScan {
        ...
        let payload = &bytes[4..];
        if crc != fuju_trace_wal::crc32(payload) {
            return FullRecordScan {
                data_bytes_read,
                ..FullRecordScan::default()
            };
        ...
        FullRecordScan {
            rows: fuju_trace_wal::decode_records(payload).unwrap_or_default(),
            data_bytes_read,
        }
    }
    ...
    fn write_index_from_segment_bytes(&self, seg: SegmentId, bytes: &[u8]) -> Option<IndexHeader> {
        ...
        if fuju_trace_wal::crc32(payload) != data_crc {
            return None;
        }
        let ranges = fuju_trace_wal::encoded_record_ranges(payload)?;
        ...
        for (row, range) in ranges.into_iter().enumerate() {
            ...
            entries.push(IndexEntry {
                ...
                len: range.len,
                record_crc: fuju_trace_wal::crc32(record),
            });
        }
...
impl SegmentStore for FileSegmentStore {
    fn flush_to_segment(&self, seg: SegmentId, records: &[WalRecord]) {
        let payload = fuju_trace_wal::encode_records(records);
        let mut buf = Vec::with_capacity(payload.len() + 4);
        buf.extend_from_slice(&fuju_trace_wal::crc32(&payload).to_le_bytes());
...
mod tests {
    ...
    fn streaming_crc32_matches_wal_crc_across_chunk_boundaries() {
        ...
        for len in 0..=bytes.len() {
            let expected = fuju_trace_wal::crc32(&bytes[..len]);
    ...
    fn point_lookup_rejects_segment_when_an_unrelated_record_is_corrupt() {
        ...
        let first = fuju_trace_wal::encoded_record_ranges(&bytes[4..]).unwrap()[0];
