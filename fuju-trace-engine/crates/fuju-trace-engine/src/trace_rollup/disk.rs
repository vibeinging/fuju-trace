...
impl DiskTraceRollup {
    ...
    /// Visit rows in cached pages without cloning every match into a second collection.
    pub(super) fn visit_matching_rows(
        &mut self,
        query: &TraceQuery,
        filter: &SearchFilter,
        mut visit: impl FnMut(&TraceAggregateRollupRow),
    ) -> Option<()> {
        let page_ids = self.matching_pages(query, filter)?;
        self.last_pages_read = page_ids.len();
        for page_id in page_ids {
...
fn page_cache_budget() -> usize {
    std::env::var("FUJU_TRACE_ROLLUP_PAGE_CACHE_BYTES")
        .ok()
...
mod tests {
    ...
    fn v5_persists_span_lifecycle_and_v4_uses_safe_fallback() {
        let base = std::env::temp_dir().join(format!(
            "fuju_trace_rollup_lifecycle_{}_{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
