...
use fuju_trace_core::event::EventType;
use fuju_trace_core::fold::{FoldedSpan, SpanFields};
...
#[derive(Default)]
pub(crate) struct RollupPageFilter {
    pub(crate) session_id: Option<u64>,
    pub(crate) span_id: Option<u64>,
    pub(crate) external_span_id: Option<String>,
    pub(crate) external_session_id: Option<String>,
}

impl RollupPageFilter {
    fn matches(&self, row: &TraceAggregateRollupRow) -> bool {
        self.session_id
            .is_none_or(|value| row.session_id == Some(value))
            && self.span_id.is_none_or(|value| row.span_id == value)
            && self
                .external_span_id
                .as_ref()
                .is_none_or(|value| row.external_span_id.as_ref() == Some(value))
            && self
                .external_session_id
                .as_ref()
                .is_none_or(|value| row.external_session_id.as_ref() == Some(value))
    }
}

#[derive(Eq, PartialEq, Ord, PartialOrd)]
struct PageSortKey {
    primary: Reverse<Option<u64>>,
    trace_id: u64,
    span_id: u64,
}

impl PageSortKey {
    fn for_row(row: &TraceAggregateRollupRow, sort_by: &str) -> Self {
        let primary = match sort_by {
            "duration" | "durationns" => row.duration_ns,
            "tokens" | "totaltokens" => {
                Some(row.input_tokens.unwrap_or(0) + row.output_tokens.unwrap_or(0))
            }
            "status" => Some(row.status.unwrap_or(0) as u64),
            _ => None,
        };
        Self {
...
impl PartialEq for PageHit {
    fn eq(&self, other: &Self) -> bool {
        self.key == other.key
    }
}
impl Eq for PageHit {}
impl PartialOrd for PageHit {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        Some(self.cmp(other))
    }
}
impl Ord for PageHit {
...
impl TraceAggregateRollupIndex {
    ...
    pub(crate) fn query_unordered(
        ...
        self.query_inner(query, filter, false)
    }
    ...
    /// Count every match, but retain and decode only rows needed for this page.
    pub(crate) fn query_page(
        &mut self,
        query: &TraceQuery,
        filter: &SearchFilter,
        page_filter: &RollupPageFilter,
        sort_by: &str,
        cursor: usize,
        limit: usize,
    ) -> Option<(Vec<FoldedSpan>, usize, ReadPlanStats)> {
        if self.dirty {
            return None;
        }
        let sort_by = sort_by.to_ascii_lowercase().replace(['_', '-'], "");
        let keep = cursor.saturating_add(limit);
        let mut top = BinaryHeap::<PageHit>::new();
        let mut candidates = 0usize;
        let mut total = 0usize;
        let mut consider = |row: &TraceAggregateRollupRow| {
            candidates += 1;
            if !page_filter.matches(row) {
                return;
            }
            total += 1;
            if keep == 0 {
            ...
            let key = PageSortKey::for_row(row, &sort_by);
            if top.len() < keep {
                top.push(PageHit {
                    key,
                    row: row.clone(),
                });
            } else if top.peek().is_some_and(|worst| key < worst.key) {
                top.pop();
                top.push(PageHit {
                    key,
                    row: row.clone(),
        ...
        let overrides = &self.rows;
        if let Some(disk) = self.disk.as_mut() {
            disk.visit_matching_rows(query, filter, |row| {
                if !overrides.contains_key(&(row.trace_id, row.span_id)) {
                    consider(row);
                }
            })?;
        }
        ...
        top.sort_unstable_by(|a, b| a.key.cmp(&b.key));
        let spans = top
            .into_iter()
            .skip(cursor)
            .map(|hit| hit.row.to_folded_span())
            .collect();
        let stats = ReadPlanStats {
            source: Some("aggregate_rollup".to_string()),
            used_filter_index: filter.needs_indexed_filter(),
            candidate_span_keys: Some(candidates),
            scanned_segments: 0,
            matched_spans: total,
            rollup_pages_read: Some(pages_read),
            rollup_pages_total: Some(pages_total),
