impl EngineJsonApi {
    ...
    fn trace_search_json(&self, body: &str, tenant: Option<u64>) -> (u16, String) {
        let value = match crate::wire::parse(body) {
            Ok(value) => value,
            Err(error) => return (400, format!(r#"{{"error":"{}"}}"#, error.replace('"', "'"))),
        };
        let parsed = self.parse_trace_search_value(&value, tenant);
        if parsed.spec.text.is_none() {
            let page_filter = crate::trace_rollup::RollupPageFilter {
                session_id: parsed.spec.session_id,
                span_id: parsed.spec.span_id,
                external_span_id: parsed.spec.external_span_id.clone(),
                external_session_id: parsed.spec.external_session_id.clone(),
            };
            if let Some((spans, total, mut read_plan)) = self.coord.trace_rollup_page(
                &parsed.query,
                &parsed.index_filter,
                &page_filter,
                &parsed.sort_by,
                ...
                read_plan.matched_spans = total;
                let items = spans.iter().map(trace_search_item_json).collect::<Vec<_>>();
                return (
                    200,
                    format!(
        ...
        sort_trace_search_page(
            &mut read.spans,
            &read.sort_by,
            read.cursor.saturating_add(read.limit),
        );
    ...
    fn trace_aggregate_json(&self, body: &str, tenant: Option<u64>) -> (u16, String) {
        ...
        let read = if parsed.spec.text.is_none() {
            match self
                .coord
                .trace_rollup_spans_unordered(&parsed.query, &parsed.index_filter)
            {
    ...
    fn storage_stats_json(&self, body: &str, tenant: Option<u64>) -> (u16, String) {
        ...
        let mut groups: std::collections::BTreeMap<Vec<String>, StorageStatsBucket> =
            std::collections::BTreeMap::new();
        if !group_by.is_empty() {
            for span in &read.spans {
                ...
                groups
                    .entry(values.clone())
                    .or_insert_with(|| StorageStatsBucket::new(values))
                    .add(span);
            }
        }
        let group_json: Vec<String> = groups
            .into_iter()
            .map(|(_, bucket)| bucket.to_json(&group_by))
            .collect();
        (
            200,
            format!(
