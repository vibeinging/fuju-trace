...
fn compare_trace_search_spans(
    ...
    let order = match sort_by {
        ...
        _ => a.trace_id.cmp(&b.trace_id),
    };
    order.then_with(|| a.span_id.cmp(&b.span_id))
}

/// Only sort the requested prefix. Exact total is counted before this call.
fn sort_trace_search_page(spans: &mut Vec<FoldedSpan>, sort_by: &str, page_end: usize) {
    let end = page_end.min(spans.len());
    if end == 0 {
        spans.clear();
        return;
    }
    ...
    if end < spans.len() {
        spans.select_nth_unstable_by(end - 1, |a, b| compare_trace_search_spans(a, b, &sort_by));
        spans.truncate(end);
    }
    spans.sort_unstable_by(|a, b| compare_trace_search_spans(a, b, &sort_by));
}
...
impl StorageStatsBucket {
    ...
    fn add(&mut self, span: &FoldedSpan) {
        self.trace_ids.insert(span.trace_id);
        self.span_count += 1;
        self.event_count += span.event_count;
        self.estimated_bytes += estimate_span_bytes(span);
    }

    fn from_spans(spans: &[FoldedSpan], values: &[String]) -> Self {
        let mut out = Self::new(values.to_vec());
        for span in spans {
            out.add(span);
        }
        out
