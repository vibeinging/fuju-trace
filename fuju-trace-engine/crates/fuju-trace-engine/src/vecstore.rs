...
pub fn append(path: impl AsRef<Path>, trace: u64, span: u64, vec: &[f32]) -> std::io::Result<()> {
    ...
    encode_record(&mut b, trace, span, vec);
    let mut f = OpenOptions::new().create(true).append(true).open(path)?;
    f.write_all(&b)?;
    f.sync_data()
}

/// 一批向量只开一次文件、同步一次；批量接口使用同一份记录格式。
pub fn append_batch(path: impl AsRef<Path>, items: &[(u64, u64, Vec<f32>)]) -> std::io::Result<()> {
    if items.is_empty() {
        return Ok(());
    }
    let capacity = items.iter().map(|(_, _, vec)| 24 + vec.len() * 4).sum();
    let mut bytes = Vec::with_capacity(capacity);
    for (trace, span, vec) in items {
        encode_record(&mut bytes, *trace, *span, vec);
    }
...
fn encode_record(b: &mut Vec<u8>, trace: u64, span: u64, vec: &[f32]) {
    let start = b.len();
    ...
    let crc = fuju_trace_wal::crc32(&b[start..]);
...
pub fn load(path: impl AsRef<Path>) -> Vec<((u64, u64), Vec<f32>)> {
    ...
    loop {
        ...
        if crc != fuju_trace_wal::crc32(&bytes[i..i + rec_len - 4]) {
            break; // 损坏 → 停
        }
...
mod tests {
    ...
    #[test]
    fn append_batch_keeps_all_records_and_last_write_wins_order() {
        let p = temp();
        append_batch(
            &p,
            &[
