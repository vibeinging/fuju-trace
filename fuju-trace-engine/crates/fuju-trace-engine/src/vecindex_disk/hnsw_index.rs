...
impl GraphIndex for DiskGraphIndex {
    ...
    fn try_index_embedding(
        ...
        self.insert(trace_id, span_id, &v)
    }
...
fn save_upper(
    ...
    let crc = fuju_trace_wal::crc32(&b);
...
fn load_upper(path: &Path) -> (HashMap<(u32, u8), Vec<u32>>, Option<(u32, u8)>) {
    ...
    if fuju_trace_wal::crc32(body) != u32::from_le_bytes(bytes[bytes.len() - 4..].try_into().unwrap()) {
