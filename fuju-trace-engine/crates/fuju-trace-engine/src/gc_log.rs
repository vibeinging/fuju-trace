...
mod tests {
    ...
    fn pending_returns_mark_without_done() {
        let tmp = std::env::temp_dir().join("fuju_trace_gctest1.log");
    ...
    fn scan_tolerates_torn_tail() {
        let tmp = std::env::temp_dir().join("fuju_trace_gctest2.log");
        let _ = std::fs::remove_file(&tmp);
        // 手写一行正常 + 一行坏（撕裂：数字残缺）
    ...
    fn scan_missing_file_is_empty() {
        let tmp = std::env::temp_dir().join("fuju_trace_gctest_nonexistent.log");
        let _ = std::fs::remove_file(&tmp);
        let entries = GcLog::scan(&tmp).unwrap();
