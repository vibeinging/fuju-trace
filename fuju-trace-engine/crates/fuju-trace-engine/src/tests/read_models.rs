...
fn single_span_detail_point_read_does_not_decode_the_full_trace() {
    ...
    static N: AtomicU64 = AtomicU64::new(0);
    let dir = std::env::temp_dir().join(format!(
        "fuju_trace_single_span_point_{}_{}",
        std::process::id(),
        N.fetch_add(1, Ordering::Relaxed)
    ));
    let _ = std::fs::remove_dir_all(&dir);
...
fn reopened_single_span_detail_loads_bloom_before_validating_segments() {
    ...
    static N: AtomicU64 = AtomicU64::new(0);
    let dir = std::env::temp_dir().join(format!(
        "fuju_trace_reopen_single_span_bloom_{}_{}",
        std::process::id(),
        N.fetch_add(1, Ordering::Relaxed)
    ));
    let _ = std::fs::remove_dir_all(&dir);
...
fn invalid_bloom_sidecar_rebuilds_once_and_persists_recovery() {
    ...
    static N: AtomicU64 = AtomicU64::new(0);
    let dir = std::env::temp_dir().join(format!(
        "fuju_trace_invalid_point_bloom_{}_{}",
        std::process::id(),
        N.fetch_add(1, Ordering::Relaxed)
    ));
    let _ = std::fs::remove_dir_all(&dir);
...
fn bloom_sidecar_bit_corruption_falls_back_without_missing_span() {
    ...
    static N: AtomicU64 = AtomicU64::new(0);
    let dir = std::env::temp_dir().join(format!(
        "fuju_trace_corrupt_point_bloom_{}_{}",
        std::process::id(),
        N.fetch_add(1, Ordering::Relaxed)
    ));
    let _ = std::fs::remove_dir_all(&dir);
...
fn bloom_sidecar_rejects_extreme_lengths_and_hash_count() {
    ...
    static N: AtomicU64 = AtomicU64::new(0);
    let dir = std::env::temp_dir().join(format!(
        "fuju_trace_oversized_point_bloom_{}_{}",
        std::process::id(),
        N.fetch_add(1, Ordering::Relaxed)
    ));
    let _ = std::fs::remove_dir_all(&dir);
    ...
    let write_and_reject = |mut payload: Vec<u8>| {
        let checksum = fuju_trace_wal::crc32(&payload);
...
fn concurrent_reopened_point_reads_share_one_bloom_load() {
    ...
    static N: AtomicU64 = AtomicU64::new(0);
    let dir = std::env::temp_dir().join(format!(
        "fuju_trace_concurrent_point_bloom_{}_{}",
        std::process::id(),
        N.fetch_add(1, Ordering::Relaxed)
    ));
    let _ = std::fs::remove_dir_all(&dir);
...
fn loading_old_snapshot_bloom_does_not_remove_new_segment_blooms() {
    ...
    static N: AtomicU64 = AtomicU64::new(0);
    let dir = std::env::temp_dir().join(format!(
        "fuju_trace_old_snapshot_bloom_merge_{}_{}",
        std::process::id(),
        N.fetch_add(1, Ordering::Relaxed)
    ));
    let _ = std::fs::remove_dir_all(&dir);
