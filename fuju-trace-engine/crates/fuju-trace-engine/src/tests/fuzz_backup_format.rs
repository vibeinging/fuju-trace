...
fn backup_snapshot_restores_consistent_data() {
    ...
    let dir = std::env::temp_dir().join(format!("fuju_trace_backup_{}", std::process::id()));
    let backup_dir = std::env::temp_dir().join(format!("fuju_trace_backup_copy_{}", std::process::id()));
...
fn format_version_check_and_migrate() {
    // §3.4：版本检查 + 迁移。
    let dir = std::env::temp_dir().join(format!("fuju_trace_migrate_{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
