...
fn map_span(sp: &Json, out: &mut Vec<WireRecord>) -> Result<(), String> {
    ...
    let display_name = first_str(attrs, &["fuju.trace.display_name", "yitrace.display_name"])
    ...
    // 会话 id：OTLP 里常是字符串（session.id / fuju.trace.session_id）。本引擎要 u64 →
    // 数字直接解析，否则确定性哈希，确保同一外部会话稳定落到同一 session_id。
    let session_id = first_session_id(
        attrs,
        &[
            "fuju.trace.session_id",
            "yitrace.session_id",
            "session.id",
    ...
    let tenant_id = first_numeric_u64(
        attrs,
        &[
            "fuju.trace.tenant_id",
            "yitrace.tenant_id",
            "tenant.id",
            "tenant_id",
        ],
...
/// 字符串会话 id → u64：纯数字直接解析，否则用 fuju-trace-core 的确定性 FNV-1a 64（不再自己抄一份哈希常量）。
fn str_to_u64(s: &str) -> u64 {
    s.parse::<u64>()
        .unwrap_or_else(|_| fuju_trace_core::event::fnv1a64(s.as_bytes()))
}
...
mod tests {
        ...
        "scopeSpans": [{
          "spans": [{
            ...
            "attributes": [
              ...
              {"key":"fuju.trace.display_name","value":{"stringValue":"  风控助手研判  "}},
    ...
    #[test]
    fn maps_fuju_trace_session_and_numeric_tenant_attrs() {
        let j = r#"{"resourceSpans":[{"scopeSpans":[{"spans":[{
            "traceId":"abc","spanId":"00000000000000aa",
            "name":"chat","startTimeUnixNano":"1","endTimeUnixNano":"2",
            "attributes":[
              {"key":"fuju.trace.session_id","value":{"stringValue":"tenant-a-session-1"}},
              {"key":"fuju.trace.tenant_id","value":{"stringValue":"42"}}
            ]
        }]}]}]}"#;
        let recs = parse_otlp_traces(j).unwrap();
    ...
    #[test]
    fn accepts_legacy_yitrace_otlp_attributes() {
        let j = r#"{"resourceSpans":[{"scopeSpans":[{"spans":[{
            "traceId":"abc","spanId":"00000000000000aa",
            "name":"chat","startTimeUnixNano":"1","endTimeUnixNano":"2",
            "attributes":[
              {"key":"yitrace.display_name","value":{"stringValue":"旧展示名"}},
              {"key":"yitrace.session_id","value":{"stringValue":"old-session"}},
              {"key":"yitrace.tenant_id","value":{"stringValue":"42"}}
            ]
        }]}]}]}"#;
        let recs = parse_otlp_traces(j).unwrap();
        assert_eq!(recs[0].display_name.as_deref(), Some("旧展示名"));
        assert_eq!(recs[0].session_id, Some(str_to_u64("old-session")));
        assert_eq!(recs[0].tenant_id, Some(42));
    }
