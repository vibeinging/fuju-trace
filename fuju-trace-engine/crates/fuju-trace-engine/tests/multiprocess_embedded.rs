...
use fuju_trace_core::fold::SpanFields;
use fuju_trace_core::ids::SegmentId;
use fuju_trace_wal::WalRecord;

use fuju_trace_engine::{
    EngineJsonApi, NewTraceAnnotation, TraceAnnotationFilter, WriteCoordinator,
};
...
fn multiple_processes_open_same_data_dir_and_write() {
    if let Some(dir) = std::env::var_os("FUJU_TRACE_MULTIPROCESS_EMBEDDED_HELPER_DIR") {
        let trace = std::env::var("FUJU_TRACE_MULTIPROCESS_EMBEDDED_HELPER_TRACE").unwrap();
    ...
    assert!(
        coord
            .metrics()
            .contains("fuju_trace_process_lock_wait_seconds_total"),
...
fn multiple_processes_write_vectors_and_reopen_searches_them() {
    if let Some(dir) = std::env::var_os("FUJU_TRACE_MULTIPROCESS_EMBEDDED_VECTOR_DIR") {
        let trace = std::env::var("FUJU_TRACE_MULTIPROCESS_EMBEDDED_VECTOR_TRACE").unwrap();
        let x = std::env::var("FUJU_TRACE_MULTIPROCESS_EMBEDDED_VECTOR_X")
            .unwrap()
            .parse::<f32>()
            .unwrap();
...
fn many_processes_incrementally_refresh_wal_tail() {
    if let Some(dir) = std::env::var_os("FUJU_TRACE_MULTIPROCESS_EMBEDDED_STRESS_DIR") {
        let worker = std::env::var("FUJU_TRACE_MULTIPROCESS_EMBEDDED_STRESS_WORKER").unwrap();
        let count = std::env::var("FUJU_TRACE_MULTIPROCESS_EMBEDDED_STRESS_COUNT")
            .unwrap()
            .parse::<usize>()
            .unwrap();
...
fn reclaim_waits_for_cross_process_reader_pin() {
    if let Some(dir) = std::env::var_os("FUJU_TRACE_MULTIPROCESS_EMBEDDED_READER_DIR") {
        let ready = PathBuf::from(
            std::env::var_os("FUJU_TRACE_MULTIPROCESS_EMBEDDED_READER_READY").unwrap(),
        );
...
fn multiple_processes_rebuild_one_missing_segment_index() {
    if let Some(dir) = std::env::var_os("FUJU_TRACE_MULTIPROCESS_INDEX_REBUILD_DIR") {
        let worker = std::env::var("FUJU_TRACE_MULTIPROCESS_INDEX_REBUILD_WORKER").unwrap();
...
fn spawn_writer(dir: &Path, trace: &str) -> Child {
    Command::new(std::env::current_exe().unwrap())
        ...
        .arg(TEST_NAME)
        .arg("--nocapture")
        .env("FUJU_TRACE_MULTIPROCESS_EMBEDDED_HELPER_DIR", dir)
        .env("FUJU_TRACE_MULTIPROCESS_EMBEDDED_HELPER_TRACE", trace)
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
        .unwrap()
...
fn spawn_reader(dir: &Path, ready: &Path) -> Child {
    Command::new(std::env::current_exe().unwrap())
        ...
        .arg("--nocapture")
        .env("FUJU_TRACE_MULTIPROCESS_EMBEDDED_READER_DIR", dir)
        .env("FUJU_TRACE_MULTIPROCESS_EMBEDDED_READER_READY", ready)
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
        .unwrap()
...
fn spawn_stress_writer(dir: &Path, worker: &str, count: usize) -> Child {
    Command::new(std::env::current_exe().unwrap())
        ...
        .arg("--nocapture")
        .env("FUJU_TRACE_MULTIPROCESS_EMBEDDED_STRESS_DIR", dir)
        .env("FUJU_TRACE_MULTIPROCESS_EMBEDDED_STRESS_WORKER", worker)
        .env(
            "FUJU_TRACE_MULTIPROCESS_EMBEDDED_STRESS_COUNT",
            count.to_string(),
        )
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
...
fn spawn_vector_writer(dir: &Path, trace: &str, x: f32) -> Child {
    Command::new(std::env::current_exe().unwrap())
        ...
        .arg("--nocapture")
        .env("FUJU_TRACE_MULTIPROCESS_EMBEDDED_VECTOR_DIR", dir)
        .env("FUJU_TRACE_MULTIPROCESS_EMBEDDED_VECTOR_TRACE", trace)
        .env("FUJU_TRACE_MULTIPROCESS_EMBEDDED_VECTOR_X", x.to_string())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
        .unwrap()
...
fn spawn_index_rebuilder(dir: &Path, worker: usize) -> Child {
    Command::new(std::env::current_exe().unwrap())
        ...
        .arg("--nocapture")
        .env("FUJU_TRACE_MULTIPROCESS_INDEX_REBUILD_DIR", dir)
        .env(
            "FUJU_TRACE_MULTIPROCESS_INDEX_REBUILD_WORKER",
            worker.to_string(),
        )
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
