...
fn ingest_dataset(coord: &Arc<WriteCoordinator>, args: &Args) -> (DatasetStats, WriteStats) {
    ...
    let flush_rows = coord.memtable_len();
    eprintln!("scale_bench: flush remaining memtable rows={flush_rows}");
...
fn render_report(report: &Report) -> String {
    ...
    let _ = writeln!(out, "# Fuju Trace Scale Bench Report");
    let _ = writeln!(out);
...
impl Args {
    fn parse() -> Self {
        ...
        let data_dir = data_dir.unwrap_or_else(|| {
            std::env::temp_dir().join(format!("fuju_trace_scale_bench_{}", std::process::id()))
        });
