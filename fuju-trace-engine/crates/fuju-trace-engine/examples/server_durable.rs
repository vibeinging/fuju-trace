//! 持久化服务（崩溃测试用）：`cargo run -p fuju-trace-engine --example server_durable -- /path/to/data`
//!
//! 和 `server` 区别：用 `open_durable`（段 + WAL + manifest + 向量索引全落盘），
//! 进程被 kill -9 后重启数据仍在。用于真·崩溃测试集成脚本。
//!
//! 也可作为最小持久化部署样板：
//!   cargo run -p fuju-trace-engine --example server_durable -- /data/fuju_trace
//!   # 然后 curl 灌/查；kill -9 后重启同样命令，数据还在。
use std::net::TcpListener;
use std::sync::Arc;

use fuju_trace_engine::{HttpIngestServer, WriteCoordinator};

fn main() {
    let dir = std::env::args()
        .nth(1)
        .unwrap_or_else(|| "./fuju-trace-data".to_string());
