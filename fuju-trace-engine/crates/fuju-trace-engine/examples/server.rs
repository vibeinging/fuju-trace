//! 可运行控制台服务：`cargo run -p fuju-trace-engine --example server`
...
use fuju_trace_engine::{evalkit, HttpIngestServer, InMemorySegmentStore, WriteCoordinator};
...
fn main() {
    ...
    if let Ok(tok) = std::env::var("FUJU_TRACE_TOKEN") {
        server = server.with_auth_token(tok);
        println!("（已开启 Bearer token 鉴权）");
    }
    let server = Arc::new(server);
    let addr = std::env::var("FUJU_TRACE_BIND").unwrap_or_else(|_| "127.0.0.1:7878".to_string());
    let listener = TcpListener::bind(&addr).expect("bind");
    println!("Fuju Trace 控制台 → http://{addr}/  （前端需先 build 并拷到 console_dist/）");
