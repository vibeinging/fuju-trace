//! 单 Span 详情点读基准。
//!
//! 跑：`cargo run --release -p fuju-trace-engine --example bench_span_detail`
//! 分别构造包含 10 / 100 / 1000 个 span 的单条 Trace，数据 flush 到真实文件段后，
//! 通过 `GET /v1/traces/:traceId/spans/:spanId` 的同一进程 JSON API 测 P50/P95。

use fuju_trace_engine::{EngineJsonApi, WireRecord, WriteCoordinator};
...
fn main() {
    ...
    for span_count in [10usize, 100, 1_000] {
        let dir = std::env::temp_dir().join(format!(
            "fuju_trace_bench_span_detail_{}_{}",
            std::process::id(),
            span_count
        ));
