...
//!
//! 跑：`cargo run --release -p fuju-trace-engine --example bench_qps [N] [dim] [queries]`
//! 默认 N=20000 span、dim=128、queries=2000。务必 `--release`（debug 慢几十倍，数字没意义）。
use fuju_trace_engine::{WireRecord, WriteCoordinator};
...
fn main() {
    ...
    let vector_batch: usize = a.get(4).and_then(|s| s.parse().ok()).unwrap_or(1).max(1);

    let dir = std::env::temp_dir().join(format!("fuju_trace_bench_qps_{}", std::process::id()));
    ...
    let tv = Instant::now();
    for start in (0..n).step_by(vector_batch) {
        let batch = (start..(start + vector_batch).min(n))
            .map(|id| (id as u64, 1, rng.vec(dim)))
            .collect();
        wc.try_index_embeddings(batch).unwrap();
    }
    ...
    println!("规模: {n} span, 向量 {dim} 维, 向量批量 {vector_batch}, 各 {queries} 次查询  | 本机单机, release");
    println!("---");
    println!(
