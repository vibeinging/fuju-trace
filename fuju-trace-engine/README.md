...
```bash
cargo test --offline                              # 156 测试(fuju-trace-core 9 + fuju-trace-engine 130 + eval_harness 6 + manifest 4 + memtable 2 + wal 5)
cargo run -p fuju-trace-engine --example demo --offline  # 灌几条银行风控假 trace,跑写入→折叠→中文搜→找相似→混合召回
cargo run -p fuju-trace-engine --example server          # 起 HTTP 摄入服务(8 线程池),curl 即可灌/查
FUJU_TRACE_TOKEN=secret cargo run ... --example server   # 开 Bearer token 鉴权
cargo test -p fuju-trace-engine --features gzip          # 含 gzip 请求体解压(可选 feature,默认离线 std-only)
cargo run -p fuju-trace-engine --release --example bench_qps  # 真实 QPS 压测(摄入/检索/建图)
```
...
| **Vortex 列式段** | ✅ | 隔离 crate `fuju-trace-segstore-vortex`;**谓词下推 + 投影下推**(聚合只读窄列、跳过大文本列) |
...
| **磁盘型多层 HNSW** | ✅ | 落盘版 HNSW(参考 Fuju Trace graph_index):底层+向量在磁盘、上层稀疏骨架常驻内存、向量按需读页走缓冲池;重启不 rebuild |
...
| crate | 干什么 |
|---|---|
| `fuju-trace-core` | 核心类型:三类不可变标识、**确定性 event_id**、不可变 Manifest(写时复制)、deletion/upgrade 对称块、**四源折叠算法**(纯函数)、RRF 融合 |
| `fuju-trace-manifest` | **正确性脊梁**:读者 pin 协议(先登记再读)、回收水位、RAII 自动注销 |
| `fuju-trace-wal` | 写前日志:文件落盘 + fsync;崩溃安全帧;自研二进制编码(表查 CRC32) |
| `fuju-trace-memtable` | 活内存表:上下界双水位 + 受 gate 的 evict(修"flush 后漏读一截") + 自动刷盘 |
| `fuju-trace-engine` | 单写者协调器、段五态生命周期、磁盘型 HNSW、BM25 倒排、四源折叠、HTTP 服务、OTLP 适配、eval、多租户、投影/谓词下推 |

## 相关 crate(工作区外,隔离重依赖)

| crate | 干什么 |
|---|---|
| `fuju-trace-segstore-vortex` | Vortex 列式段存储(实现 SegmentStore);谓词下推 + 投影下推 |
| `fuju-trace-tokenizer-jieba` | 可选外部分词 FFI 适配层；**引擎默认用纯 Rust 词级分词** |
| `fuju-trace-vecindex-graph` | 可选外部图索引 FFI 适配层；**引擎默认用自研磁盘型 HNSW** |
...
- `docs/2026-06-22_fuju-trace-产品说明.md` — 产品定位 / 三条护城河 / 竞品对比
...
- `docs/design/2026-06-17_fuju-trace-segment-snapshot-hardened.md` — 并发设计加固稿(段生命周期 + 快照)
