...
> 本仓库是 Fuju Trace，负责追踪 SDK、回放控制台和 TraceDB。优化实验、Benchmark、独立验收和 Runtime 属于独立 `fuju-rsi` 仓库。Trace 引擎保持 Rust 标准库零外部依赖，RSI 通过可选插件使用公开打点接口。

> 本文件给 AI 编程助手（Cursor / Claude Code / ZCode / 其他 agentic 工具）读，也供人类开发者参考。
> 它说清：**这个项目是什么、怎么构建测试、改代码要注意什么、以及如何对接 Fuju Trace（灌 trace / 查询 / 检索）**。
...
**Fuju Trace** 是给 AI Agent 用的本地 trace SDK、回放控制台和嵌入式 TraceDB。Rust 引擎主体是自研的，可以作为独立 HTTP 服务运行，也可以被 Node / Electron / Python / Rust 应用嵌入到进程内。把 Agent（多轮对话、调工具、多 agent 协作）跑出来的 trace 灌进来，提供：
...
- **引擎主体零外部依赖、只用 Rust 标准库**。`cargo test --offline` 必须离线可过。重依赖（Vortex、N-API、PyO3 等）隔离在独立目录 / crate（`fuju-trace-segstore-vortex`、`fuju-trace-node`、`fuju-trace-db-python` 等），**不要把外部 crate 拉进 `fuju-trace-engine`**。
...
```
fuju-trace-engine/              # 引擎（Rust workspace，std-only 零依赖）— 当前承重代码
│   └── crates/
│       ├── fuju-trace-core/            # 核心类型：ids、确定性 event_id、不可变 Manifest、折叠算法
│       ├── fuju-trace-manifest/        # 单写者-多读者：快照 pin 协议、回收水位（正确性脊梁）
│       ├── fuju-trace-wal/             # 写前日志：fsync、崩溃安全帧、二进制编码
│       ├── fuju-trace-memtable/        # 活内存表：双水位 + 受 gate 的 evict
│       └── fuju-trace-engine/          # 协调器、四源折叠读、检索、eval、HTTP/OTLP、控制台
│           └── examples/       # demo / server / bench_qps / eval_harness
fuju-trace-segstore-vortex/     # Vortex 列式段（工作区外，隔离重依赖）
fuju-trace-tokenizer-jieba/     # cppjieba FFI（可选；引擎默认用纯 Rust ChineseTokenizer）
fuju-trace-vecindex-graph/      # graph_index FFI（可选；引擎默认用自研磁盘 HNSW）
fuju-trace-sdk/                 # 打点 SDK
│   ├── python/                  # fuju_trace 包（pyproject.toml 已配，纯标准库）
│   ├── typescript/              # @fuju/trace-sdk（tsconfig + build 已配）
│   └── rust/                    # fuju-trace crate（纯 std Rust 打点 SDK）
fuju-trace-node/                # @fuju/trace-db：Node/Electron 嵌入式 DB（N-API，独立于 engine workspace）
│   └── npm/                     # NAPI-RS optional platform packages（darwin/linux/win）
fuju-trace-db-python/           # fuju-trace-db：Python 嵌入式 DB（PyO3/maturin，可选 FastAPI server）
fuju-trace-db-rs/               # fuju-trace-db：Rust 嵌入式 DB crate
fuju-trace-console/             # 控制台前端（React + Vite + TS，构建产物内嵌进引擎单二进制）
...
```bash
cd fuju-trace-engine
cargo test --offline                    # 全测试（含并发压测 + HTTP 往返 + ANN 召回 + 重启不丢）
cargo run -p fuju-trace-engine --example demo   # 可运行 demo：灌数据 → 折叠 → 中文搜 → 向量 → 混合召回
cargo run -p fuju-trace-engine --example server # 起 HTTP 服务（:7878，自带 eval 种子数据）
cargo run -p fuju-trace-engine --example bench_qps --release  # 真实 QPS 压测（务必 --release）
cargo run -p fuju-trace-engine --example scale_bench --release -- --phase query --data-dir /path/to/data --verify-search --verify-source-index  # 查询优化 + 原始事件双重对账
```

- **测试必须 `--offline` 能过**（守零依赖原则）。release 才跑 bench（debug 慢几十倍，数字无意义）。
- 改了 HTTP/控制台后，要重新构建前端并内嵌：`cd fuju-trace-console && VITE_API=http npm run build && rm -rf ../fuju-trace-engine/crates/fuju-trace-engine/console_dist && cp -r dist ../fuju-trace-engine/crates/fuju-trace-engine/console_dist`。
- 可选：`FUJU_TRACE_TOKEN=secret cargo run ... --example server` 开 Bearer 鉴权；`cargo test -p fuju-trace-engine --features gzip` 含 gzip 解压。

**外部 crate（隔离的重依赖，按需构建）：**

```bash
cd fuju-trace-segstore-vortex && cargo build     # Vortex（需 Rust 1.91+）
cd fuju-trace-tokenizer-jieba && cargo build     # jieba FFI（默认 mock，--features link 接真库）
cd fuju-trace-vecindex-graph && cargo build      # graph_index FFI（同上）
```
...
```bash
# Python
cd fuju-trace-sdk/python && python -m build      # 出 wheel/sdist（pyproject.toml 已配）
python -m pytest                              # 跑测试
# TypeScript
cd fuju-trace-sdk/typescript && npm install && npm run build   # tsc 出 dist/
npm test                                      # tsx 跑测试
# Rust
cd fuju-trace-sdk/rust && cargo test --offline   # 纯 std 打点 SDK
```

**嵌入式 DB 包：**

```bash
cd fuju-trace-db-python && python -m pytest      # Python embedded DB + FastAPI/CLI + 真实多进程 worker 测试
cd fuju-trace-db-rs && cargo test --offline      # Rust embedded DB crate
...
**Node / Electron 嵌入式 DB（`@fuju/trace-db`）：**
...
```bash
cd fuju-trace-console && npm install && npm run dev     # 开发服务 :5180（默认 mock 数据）
...
- **零依赖**：要在 `fuju-trace-engine` 引入外部 crate，先想清楚能不能放进独立的外部 crate。引擎本体只 std。
- **测试是承重的**：每个不变量都有"真会失败的测试"（崩溃重放、召回对标、确定性 event_id 跨语言对账）。改逻辑前先看相关测试，改完跑全量。
- **命名**：crate `fuju-trace-*`，Rust 标识符 `fuju_trace_`，Prometheus 指标 `fuju_trace_*`，环境变量 `FUJU_TRACE_*`，Python 发布包 `fuju-trace`（导入名 `fuju_trace`），TS 包 `@fuju/trace-sdk`，嵌入式 DB 包 `@fuju/trace-db`。顶层目录 `fuju-trace-*`。**不要引入旧前缀。**
- **N-API 包隔离**：`fuju-trace-node/` 可以依赖 NAPI-RS；这些依赖不得进入 `fuju-trace-engine`。`@fuju/trace-db` 只通过 Rust engine API 打开数据目录，不允许 JS 直接解析 WAL/manifest/segment 文件。嵌入式查询走 `EngineJsonApi` 进程内调用，不启动本地 HTTP server、不走 TCP socket。
- **恢复路径不能 eager load 大索引**：持久库 clean reopen 只恢复 manifest、WAL checkpoint 和控制状态；`trace_rollup.dat`、`filter_attrs.dat`、`bm25.dat`、`segment_bloom.dat` 按第一次相关查询加载。`trace_rollup.dat` v3 只加载页目录，按 tenant/project/trace 范围读页，并保留 v2 读取兼容；`filter_attrs.dat` v3 只加载行和 posting 目录；`bm25.dat` v4 只加载文档长度、确定性 event_id 表和词/块目录，按 block-max 上界决定是否读 128 条 postings。同一 event_id 在重试、WAL 重放和重开后只能索引一次；修改这条路径必须跑 `--verify-source-index`。默认 segment 的 `seg-*.idx` 保存 `(trace_id, span_id) -> byte offset`，启动时不加载；首次点查会校验索引并重算整个 segment CRC，损坏段在点查和顺序读下都必须拒绝。索引缺失或损坏会从对应 `seg-*.dat` 重建；它不是数据源。`segment_bloom.dat` v2 使用全文件 CRC；v1、缺失或损坏时，第一次点查会从真实 segment 重建并原子写回，迁移读取量必须计入 `readPlan`，并标记 `segment_bloom_migrated`。BM25 与 attrs 缓存默认各受 64 MB 预算约束，按内存容器 capacity 计费；无过滤 BM25 结果缓存默认 16 MB，由 `FUJU_TRACE_BM25_QUERY_CACHE_BYTES` 控制，写入后必须失效；查询候选集合另由 `FUJU_TRACE_BM25_FILTER_SET_BUDGET_BYTES` 控制。`readPlan.indexBytesRead/dataBytesRead/indexesValidated/indexesRebuilt` 必须保留，不能只报解码行数。改恢复逻辑后必须跑 `scale_bench --phase open`，不能让启动重新随 span 数线性增长。
- **npm 发布**：`@fuju/trace-db` root 包只放 JS 入口（ESM + CommonJS）和类型声明；native binary 放在 `npm/*` 平台 optional packages。正式发布前必须跑 `npm run release:artifacts` + `npm run release:prepublish` + `npm run pack:check`，并先发布平台包再发布 root 包。
- **版本与产物验证**：tag、Node root/平台包、Python、TypeScript、Rust 包和 lockfile 必须通过 `scripts/check_release_versions.py`。`pack:local` 默认只打 root + 当前平台，避免误带本机残留的旧平台二进制；只有确认所有平台二进制都是本次构建时才设置 `FUJU_TRACE_PACK_ALL_PLATFORMS=1`。Python DB wheel 必须用 `scripts/verify_python_db_wheel.py` 在干净环境验证 embedded、reopen、FastAPI 和 CLI server。
...
## 5. 对接 Fuju Trace（灌 trace / 查询 / 检索）

### 5.1 起服务

```bash
cd fuju-trace-engine && cargo run -p fuju-trace-engine --example server
...
```python
from fuju_trace import Tracer, HttpExporter

# 指向 Fuju Trace 服务；event_id 与引擎逐字节一致，重传/崩溃重放自动去重
...
```typescript
import { Tracer, HttpExporter } from "@fuju/trace-sdk";
...
```rust
use fuju_trace::{HttpExporter, TraceOptions, Tracer};
...
Python 项目如果只想打点上报，用 `fuju_trace` SDK + `HttpExporter`。如果还要在本地搜索 trace，安装 `fuju-trace-db`，再通过 `fuju_trace.connect(path=...)` 打开本地目录：
...
`connect(url=...)` 返回 HTTP client，`connect(path=...)` 返回本地 embedded DB handle。`fuju-trace-db` 也提供可选 FastAPI router 和 `fuju-trace-db serve` CLI。embedded 模式支持同机多个进程打开同一个本地 data dir；引擎内部用 data-dir 锁串行化 open/write，并用 reader pin 保护跨进程快照回收。服务端用 `init_fuju_trace(...)` 时，可用 `runtime.health()` 查看 `enabled`、`mode`、`data_dir`、队列、drop、`last_error` 和锁等待；直接用 `FujuTraceDB` 时可用 `db.lock_metrics()` 排查是否在等 DB 锁。多机器、网络文件系统或跨主机容器共享同一个 data dir 时，改用一个 Fuju Trace server，其他 worker 走 HTTP。`SpoolDbExporter` 是削峰、隔离 native 包或避免请求路径等待 DB 锁的可选方案，不再是同机多 worker 的必需方案。
...
```bash
npm install @fuju/trace-db
```

```typescript
import { FujuTraceDB, createSpanEventBuilder } from "@fuju/trace-db";

const db = await FujuTraceDB.open({ dataDir: "./data", tenantId: 1 });
...
2. **不要在 `fuju-trace-engine` 加外部依赖**；要加重依赖，放进独立外部 crate。
3. **改 event 编码 / 折叠逻辑 / 检索算子**，必须更新或新增对应测试（这些是承重不变量）。
4. **改了前端**，记得重新 build 并拷到 `console_dist/`（否则引擎内嵌的是旧版）。
5. **改了 `fuju-trace-node/`**，至少跑 `npm run build && npm test`；如果影响发布结构，同步更新 `fuju-trace-node/README.md` 和本文件。
6. **改了 `fuju-trace-sdk/rust/`、`fuju-trace-db-python/` 或 `fuju-trace-db-rs/`**，至少跑对应包测试；如果影响 embedded 多进程、`connect(path=...)`、spool、FastAPI/CLI 或包边界，再跑 `./scripts/package_mode_eval.sh`。Python embedded 多 worker 必须保留真实 `multiprocessing` 子进程测试，不要只用同进程多 handle 代替。
