...
use fuju_trace_db::{FujuTraceDb, OpenOptions, SearchQuery, SpanEndOptions, SpanEventBuilder};
...
fn temp_dir(name: &str) -> std::path::PathBuf {
    let nanos = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    std::env::temp_dir().join(format!("fuju_trace_db_rs_{name}_{nanos}"))
}

#[test]
fn rust_embedded_db_ingests_searches_and_reads_details() {
    let dir = temp_dir("roundtrip");
    let db = FujuTraceDb::open_with_options(OpenOptions::new(&dir).tenant_id(1)).unwrap();
...
fn rust_embedded_db_allows_multiple_handles_with_serialized_writes() {
    ...
    let mut first = FujuTraceDb::open(&dir).unwrap();
    let mut second = FujuTraceDb::open(&dir).unwrap();
    ...
    first.close().unwrap();
    second.close().unwrap();
    let reopened = FujuTraceDb::open(&dir).unwrap();
...
fn rust_embedded_db_route_json_reports_closed_and_request_errors() {
    ...
    let mut db = FujuTraceDb::open(&dir).unwrap();
