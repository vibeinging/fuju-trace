...
use fuju_trace_db::{
    FujuTraceDb, JsonValue, OpenOptions, SearchQuery, SpanEndOptions, SpanEventBuilder,
};
...
fn temp_dir(name: &str) -> std::path::PathBuf {
    let nanos = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    std::env::temp_dir().join(format!("fuju_trace_db_rs_eval_{name}_{nanos}"))
}

#[test]
fn eval_rust_embedded_db_preserves_external_ids_attrs_and_reopens() {
    let dir = temp_dir("durable_attrs");
    let mut db = FujuTraceDb::open_with_options(OpenOptions::new(&dir).tenant_id(7)).unwrap();
    ...
    events
        ...
        .start_span_with(
            "agent-span-rs-uuid",
            fuju_trace_db::SpanStartOptions::new("退款审核")
    ...
    let reopened = FujuTraceDb::open_with_options(OpenOptions::new(&dir).tenant_id(7)).unwrap();
...
fn eval_rust_embedded_db_keeps_tenant_boundary_on_helpers_and_raw_route() {
    ...
    let db = FujuTraceDb::open_with_options(OpenOptions::new(&dir).tenant_id(11)).unwrap();
