...
Embedded Fuju Trace DB for Rust agents.

This crate is the Rust equivalent of `@fuju/trace-db` and `fuju-trace-db` for
Python. It opens a Fuju Trace data directory in-process, calls the Rust engine
through `EngineJsonApi`, and does not start an HTTP server or parse storage
files in application code.

If a Rust app only needs to emit traces to a running Fuju Trace service, use the
pure SDK crate under `fuju-trace-sdk/rust` instead:

```toml
[dependencies]
fuju_trace = { path = "../fuju-trace-sdk/rust" }
```

Use this `fuju-trace-db` crate only when the Rust app needs local searchable
storage.
...
```toml
[dependencies]
fuju-trace-db = { path = "../fuju-trace-db-rs" }
```

The crate is not published yet. A real release should publish this crate as
`fuju-trace-db` and keep the internal `fuju-trace-engine` API behind this wrapper.

## Usage

```rust
use fuju_trace_db::{OpenOptions, SearchQuery, SpanEndOptions, SpanEventBuilder, FujuTraceDb};

fn main() -> Result<(), Box<dyn std::error::Error>> {
    let db = FujuTraceDb::open_with_options(OpenOptions::new("./data").tenant_id(1))?;
