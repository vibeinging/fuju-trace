//! fuju-trace-tokenizer-jieba —— 团队 **jieba 词级分词** 接入引擎的 [`fuju_trace_engine::Tokenizer`] 接缝。
//!
...
//! use fuju_trace_tokenizer_jieba::JiebaTokenizer;
...
use fuju_trace_engine::Tokenizer;
...
impl std::fmt::Display for JiebaError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            JiebaError::OpenFailed => {
                write!(f, "vexjieba_open 加载词典失败（路径不存在或词典损坏）")
            }
        }
    }
}
...
mod mock {
    ...
    #[no_mangle]
    unsafe extern "C" fn vexjieba_cut(
        _handle: *mut c_void,
        text: *const c_char,
        text_len: usize,
    ) -> *mut c_char {
