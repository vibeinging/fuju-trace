...
安装 extra 会带上 `fuju-trace-vexdb` 和通用 `psycopg2-binary>=2.9.5,<3`；已有兼容版本会被复用。若项目自行提供兼容的 `psycopg2` 驱动，可单独安装 `fuju-trace-vexdb==0.1.10`。
...
这三个 SQL 插件可分别通过 `fuju-trace[sqlite]`、`fuju-trace[duckdb]`、`fuju-trace[postgresql]` 安装。它们共用事件幂等、事务内折叠、租户隔离、会话/Trace 点读和精确属性过滤；文本查询暂用数据库 `LIKE` 子串匹配，**没有 BM25 或向量检索**。大数据量检索请先评估查询耗时；需要原生 BM25 和向量检索时选 VexDB。

| 数据库 | 连接参数 | 适合的场景 |
| --- | --- | --- |
| SQLite | `connect(sqlite_path="./trace.sqlite", tenant_id=1, initialize=True)` | 单机应用，文件型数据库 |
| DuckDB | `connect(duckdb_path="./trace.duckdb", tenant_id=1, initialize=True)` | 单进程写入、偏分析的本地应用 |
| PostgreSQL | `connect(postgresql_dsn="...", tenant_id=1, initialize=True)` | 已有 PostgreSQL 实例的服务 |
...
0.1.10 已在 PyPI 发布 `fuju-trace`、`fuju-trace-vexdb` 和 `fuju-trace-sql`，按需要安装基础 SDK 的 extra。历史的 [0.1.9 发版报告](docs/reports/2026-09-25_serverless-python-release.md)和 [SQL 真实库测试报告](docs/reports/2026-09-25_sql-adapter-real-tests.md)记录了验证范围。
