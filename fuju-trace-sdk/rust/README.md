# fuju_trace SDK for Rust

Pure Rust instrumentation SDK for Fuju agents.

Use this crate when a Rust app only needs to emit trace events to a running
Fuju server. Use `fuju-trace-db` when the app needs an embedded local TraceDB.
...
```rust
use fuju_trace::{HttpExporter, SpanOptions, TraceOptions, Tracer};
