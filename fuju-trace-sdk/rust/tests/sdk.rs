...
use fuju_trace::{
    event_id, CollectingExporter, EventType, Exporter, FujuTraceError, HttpExporter, SpanEvent,
    SpanOptions, TraceOptions, Tracer,
};
...
fn tracer_emits_start_log_end_and_parent_child() {
    ...
    let root_start = events
        .iter()
        .find(|event| {
            event.event_type == EventType::SpanStart && event.span_name.as_deref() == Some("root")
        })
        .unwrap();
    let child_start = events
        .iter()
        .find(|event| {
            event.event_type == EventType::SpanStart && event.span_name.as_deref() == Some("child")
        })
        .unwrap();
...
fn result_api_marks_span_failed_when_closure_returns_error() {
    let exporter = CollectingExporter::default();
    let mut tracer = Tracer::with_exporter(exporter, 1);
    let result: fuju_trace::Result<()> = tracer.trace_result("risk review", |trace| {
        trace.span_result("LLM check", |span| {
            span.log("before failure")?;
            Err(FujuTraceError::InvalidUrl("domain failure".to_string()))
        })
    });
