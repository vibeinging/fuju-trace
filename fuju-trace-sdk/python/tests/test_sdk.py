...
from fuju_trace import (  # noqa: E402
    BufferedDbExporter,
    CollectingExporter,
    DbExporter,
    EventType,
...
def test_batch_exporter_hands_off_whole_batch_once():
    ...
    from fuju_trace.exporter import BatchExporter, Exporter  # noqa: E402
...
def test_init_fuju_trace_buffered_runtime_opens_db_and_closes_it():
    class FakeDb:
        def __init__(self):
            self.calls = []
            self.closed = False
    ...
    old_module = sys.modules.get("fuju_trace_db")
    sys.modules["fuju_trace_db"] = types.SimpleNamespace(FujuTraceDB=FakeFujuTraceDB)
    try:
        runtime = init_fuju_trace(
            path="./data",
            tenant_id=5,
            node_id=1,
            flush_interval=0.01,
        ...
        if old_module is None:
            sys.modules.pop("fuju_trace_db", None)
        else:
            sys.modules["fuju_trace_db"] = old_module
...
def test_init_fuju_trace_fail_open_returns_noop_runtime():
    real_import = builtins.__import__

    def fake_import(name, *args, **kwargs):
        if name == "fuju_trace_db":
            raise ImportError("missing fuju_trace_db")
        return real_import(name, *args, **kwargs)

    try:
        builtins.__import__ = fake_import
        runtime = init_fuju_trace(path="./data", node_id=1, fail_open=True, register_atexit=False)
    ...
    assert "pip install fuju-trace-db" in str(runtime.error)
    health = runtime.health()
    ...
    assert "pip install fuju-trace-db" in health["last_error"]


def test_init_fuju_trace_spool_mode_writes_ready_file():
    from pathlib import Path  # noqa: E402
...
def test_cli_consume_spool_once_writes_embedded_db():
    ...
    import io  # noqa: E402
    from pathlib import Path  # noqa: E402
    from fuju_trace.cli import main  # noqa: E402
    from fuju_trace.event import SpanEvent  # noqa: E402
...
def test_connect_path_without_embedded_package_has_install_hint():
    ...
    try:
        ...
        try:
            ...
            raise AssertionError("connect(path=...) should fail when fuju-trace-db is missing")
    finally:
        builtins.__import__ = real_import
