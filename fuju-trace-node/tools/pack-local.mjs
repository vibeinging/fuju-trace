...
const packAllPlatforms = process.env.FUJU_TRACE_PACK_ALL_PLATFORMS === "1";
...
function cleanLabel(value) {
  ...
  if (!label) {
    throw new Error("FUJU_TRACE_PACK_LABEL must contain at least one alphanumeric, '.', '_' or '-' character");
  }
  return label;
}
...
const packLabel = cleanLabel(
  process.env.FUJU_TRACE_PACK_LABEL ??
    (gitShortCommit
...
if (localPlatform) {
  const binary = `fuju-trace-db.${localPlatform}.node`;
...
console.log(`\nPacked local Fuju Trace npm artifacts into ${dist}`);
console.log(`Immutable tarball label: ${packLabel}`);
if (gitDirty && !process.env.FUJU_TRACE_PACK_LABEL) {
