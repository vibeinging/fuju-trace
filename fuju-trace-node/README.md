...
Embedded Fuju database for Node.js and Electron.
...
`@fuju/trace-db` is the JavaScript entry package. Native binaries are published as
optional per-platform packages such as `@fuju/trace-db-darwin-arm64` and
`@fuju/trace-db-linux-x64-gnu`, so users only download the binary for their
platform.
...
This writes `@fuju/trace-db` plus the current platform package to
`fuju-trace-node/dist/*.tgz`. Set `FUJU_TRACE_PACK_ALL_PLATFORMS=1` only after a
...
the exact files. Set `FUJU_TRACE_PACK_LABEL=<release-id>` when the consuming repo
...
{
  "dependencies": {
    "@fuju/trace-db": "file:vendor/fuju-trace-db-0.1.4-g1a2b3c4d5e6f.tgz",
    "@fuju/trace-db-darwin-x64": "file:vendor/fuju-trace-db-darwin-x64-0.1.4-g1a2b3c4d5e6f.tgz"
  }
}
```

Do not keep overwriting a shared `fuju-trace-db-0.0.1.tgz`. If the payload changes,
...
- AgenticData server builds stay on one native architecture at a time. The
  current server baseline is x64: Node, DuckDB, Fuju native, and sqlite native
  must all be x64.
- Do not mix an arm64 Fuju native package with x64 DuckDB/sqlite, or the
...
- AgenticData local development may still use macOS arm64 artifacts
  (`@fuju/trace-db` root tarball + `@fuju/trace-db-darwin-arm64` tarball), but that is
...
This package does not read Fuju files directly. It embeds the Rust engine in
...
Internally it calls Fuju's `EngineJsonApi` in-process; it does not start an
...
directory. Fuju serializes open/write paths with internal data-dir locks and
...
```json
{
  "build": {
    "asarUnpack": [
      "**/*.node",
      "node_modules/@fuju/trace-db*/**/*"
    ]
  }
}
```

Do not tree-shake or prune `@fuju/trace-db-*` optional platform packages from the
...
process.env.NAPI_RS_NATIVE_LIBRARY_PATH = require("path").join(
  ...
  "native",
  "fuju-trace-db.darwin-arm64.node",
);
```

Open `FujuTraceDB` only from Electron's main process and expose specific IPC
...
`npm run build` produces a local `fuju-trace-db.<platform>.node` file for the
...
That eval builds and tests `@fuju/trace-db` together with the Python, Rust, and
...
The root `@fuju/trace-db` package declares those platform packages as
