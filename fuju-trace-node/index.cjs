const { NativeFujuTraceDB } = require("./native.cjs");
...
class FujuTraceDB {
  ...
  static async open(pathOrOptions) {
    ...
    if (!options?.dataDir) {
      throw new Error("FujuTraceDB.open requires a dataDir");
    }
    ...
    return new FujuTraceDB(new NativeFujuTraceDB(options.dataDir), options);
  }
  ...
  async indexEmbeddings(items, options = {}) {
    ...
    for (let i = 0; i < list.length; i += 1) {
      ...
      traceIds.push(String(traceId));
      spanIds.push(String(spanId));
    }
    this.#native.indexEmbeddings(traceIds, spanIds, vectors);
  ...
  async #embedQuery(text) {
    if (!this.#embedder) throw new Error("FujuTraceDB.open requires embedder for semantic/hybrid search or text embedding indexing");
  ...
  async #embedDocuments(texts) {
    if (!this.#embedder) throw new Error("FujuTraceDB.open requires embedder for text embedding indexing");
  ...
  #ensureOpen() {
    if (this.#closed) {
      throw new Error("FujuTraceDB is closed");
    }
  }
}

module.exports = { FujuTraceDB, SpanEventBuilder, createSpanEventBuilder };
