...
const { FujuTraceDB, createSpanEventBuilder } = require("../index.cjs");

(async () => {
  const dir = await mkdtemp(join(tmpdir(), "fuju-trace-node-cjs-"));
  ...
  try {
    ...
    const batch = await db.indexEmbeddings([{ traceId: 11, spanId: 22, vector: [0, 0] }]);
