...
import { FujuTraceDB, createSpanEventBuilder } from "../index.js";
...
const dir = await mkdtemp(join(tmpdir(), "fuju-trace-node-"));

try {
  await assert.rejects(
    () => FujuTraceDB.open({ dataDir: join(dir, "read-only"), readOnly: true }),
  ...
  const secondDb = await FujuTraceDB.open({ dataDir: dir, tenantId: 1 });
  ...
  const embeddingDb = await FujuTraceDB.open({
  ...
  const reopened = await FujuTraceDB.open({ dataDir: dir, tenantId: 1 });
