...
fn status_error(status: u16, body: String) -> Error {
    napi_err(format!(
        "Fuju Trace request failed: status={status} body={body}"
    ))
}

#[napi(js_name = "NativeFujuTraceDB")]
pub struct NativeFujuTraceDb {
...
impl NativeFujuTraceDb {
    ...
    pub fn new(data_dir: String) -> Result<Self> {
        ...
        let coord = WriteCoordinator::open_durable(&dir)
            .map_err(|e| napi_err(format!("open Fuju Trace data dir failed: {e}")))?;
        coord.recover();
    ...
    pub fn index_embedding(
        ...
        self.coord
            .try_index_embedding(trace_id, span_id, vector)
            .map_err(|error| napi_err(format!("index embedding failed: {error}")))
    }

    #[napi(js_name = "indexEmbeddings")]
    pub fn index_embeddings(
        ...
        self.ensure_open()?;
        if trace_ids.len() != span_ids.len() || trace_ids.len() != embeddings.len() {
            return Err(napi_err("indexEmbeddings arrays must have equal lengths"));
        }
        let mut items = Vec::with_capacity(trace_ids.len());
        for (i, ((trace_id, span_id), embedding)) in trace_ids
            .into_iter()
            .zip(span_ids)
            .zip(embeddings)
            .enumerate()
        {
            if embedding.is_empty() {
                return Err(napi_err(format!("embedding[{i}] must not be empty")));
            }
            let trace_id = parse_id_or_hash(&trace_id)?;
            let span_id = parse_id_or_hash(&span_id)?;
            let mut vector = Vec::with_capacity(embedding.len());
            for (j, value) in embedding.into_iter().enumerate() {
                if !value.is_finite() {
                    return Err(napi_err(format!("embedding[{i}][{j}] must be finite")));
                }
                vector.push(value as f32);
            }
            items.push((trace_id, span_id, vector));
        }
        self.coord
            .try_index_embeddings(items)
            .map_err(|error| napi_err(format!("index embeddings failed: {error}")))
    }
    ...
    fn ensure_open(&self) -> Result<()> {
        if self.closed {
            Err(napi_err("FujuTraceDB is closed"))
        } else {
            Ok(())
        }
    }
...
impl Drop for NativeFujuTraceDb {
    fn drop(&mut self) {
        if !self.closed {
            self.coord.flush_memtable();
            self.closed = true;
