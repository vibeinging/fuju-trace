...
> Fuju 的主要功能都通过一套 JSON API 暴露。HTTP server 调的是 `/v1/*` 端点；Node / Python / Rust 嵌入式 DB 包也是进程内调用同一层 `EngineJsonApi`。**自带控制台前端没有特权**——它和任何第三方前端调的是同一套 API。想写自己的前端 / Dashboard / Agent 后端，照着本文即可。
>
> 字段契约直接取自 `fuju-trace-engine/crates/fuju-trace-engine/src/http.rs` 的实现，不是从前端反推。
...
Fuju 有两类运行方式：独立服务和嵌入式。嵌入式目前有 Node / Python / Rust 三种包，但字段契约是一套：

| 方式 | 入口 | 是否走 HTTP socket | 适合谁 |
|---|---|---|---|
| 独立服务 | `cargo run -p fuju-trace-engine --example server` 后调 `/v1/*` | 是 | 自己写前端、已有服务要集中上报、接 OTLP/OpenInference |
| Node / Electron 嵌入式 | `@fuju/trace-db` | 否 | Node 后端、Electron main process，要像 SQLite/Chroma 一样本地打开 |
| Python 嵌入式 | `fuju-trace-db` / `import fuju_trace_db` | 否 | Python agent 或本地工具要直接写入并搜索 |
| Rust 嵌入式 | `fuju-trace-db` crate | 否 | Rust agent 或本地服务要直接嵌入 |
...
1. **只是打点上报**：优先用 `fuju_trace` Python SDK、`@fuju/trace-sdk` 或 Rust `fuju_trace` crate。
2. **已经有 OTel/OpenInference**：直接把 OTLP/HTTP JSON 发到 `POST /v1/traces`。
3. **应用内需要本地搜索和 trace 详情**：用 `@fuju/trace-db`、`fuju-trace-db` Python 包或 Rust crate。
4. **自己写 UI / 服务**：直接调 `/v1/*`。

已稳定的能力包括摄入、重启恢复、search、trace/span detail、attrs 过滤、read-model rollup、持久 BM25 缓存、annotation/dataset/retention 基座。`@fuju/trace-db` 侧已支持外部 embedding 回调：engine 不调用模型，调用方通过 `embedQuery` / `embedDocuments` 生成 vector 后写入或查询；普通 `search({ text })` 不会触发模型调用。attrs postings、rollup 页、BM25 block-max 和默认 segment 点查都已支持按目录定位、按需磁盘读取和固定预算缓存，并完成百万 span 冷启动与查询基线。仍在路线图里的上量项包括独立 loop/task/trajectory 索引和大结果聚合的流式执行。

---

## Node embedding 回调

`@fuju/trace-db` 不内置任何 embedding 模型。调用方在打开本地库时传入回调，Fuju 只接收回调产出的向量并写入磁盘图索引。

```ts
const db = await FujuTraceDB.open({
  dataDir: "./data",
  embedder: {
...
- **配了 token**（`FUJU_TRACE_TOKEN=secret cargo run ... --example server`）：`/v1/*` API 请求须带 `Authorization: Bearer <token>` 头，精确匹配，否则 401。控制台静态页面 `GET /` 仍可匿名加载，页面里的 API 请求再带 token。
...
| `localStorage["fuju.trace.tenantId"]` | 浏览器运行时设置租户（未配置 `VITE_TENANT_ID` 时生效） |
...
- Fuju 会校验并保存 value 的 JSON 字面量；search、trace、span detail 返回时恢复成相同 JSON 形态。
...
| `fuju.trace.display_name` / legacy `yitrace.display_name` | `display_name` |
| `input.value` / `gen_ai.prompt` | `input_text` |
| `output.value` / `gen_ai.completion` | `output_text` |
| `fuju.trace.session_id` / legacy `yitrace.session_id` / `session.id` / `gen_ai.conversation.id` / `session_id` | `session_id` |
| `fuju.trace.tenant_id` / legacy `yitrace.tenant_id` / `tenant.id` / `tenant_id` | direct ingest 的 `tenant_id`；HTTP 摄入仍由 `X-Tenant-Id` 覆盖 |
...
返回 Prometheus 文本格式（`# HELP` / `# TYPE` / 值），可直接被 Prometheus 抓、Grafana 出看板。指标：`fuju_trace_manifest_version`、`fuju_trace_segments_live`、`fuju_trace_memtable_rows`、`fuju_trace_segments_dead`、`fuju_trace_readers_active`、`fuju_trace_wal_committed_tail`、`fuju_trace_flush_threshold`、`fuju_trace_read_model_rollup_ready`、`fuju_trace_read_model_filter_ready`、`fuju_trace_read_model_search_ready`、`fuju_trace_filter_attrs`、`fuju_trace_filter_attr_postings`、`fuju_trace_filter_attr_disabled_postings`、`fuju_trace_fold_cache_entries`、`fuju_trace_seg_bloom_count`、`fuju_trace_datasets`。三个 `fuju_trace_read_model_*_ready` 在 clean reopen 后先为 `0`，第一次相关查询加载完成后变成 `1`。
...
这组端点用于把 trace 数据进一步筛选、聚合和估算空间。常用过滤先走 attrs sidecar。`filter_attrs.dat` v3 把 row、row directory、postings 和 posting directory 分开，查询只读取命中的字段和值。`trace_rollup.dat` v3 按 tenant、`project_id` 和 trace 范围切成最多 4096 行的页，打开只读页目录，查询只读可能命中的页；它会保存 `span_name/display_name`，并兼容读取 v2。页缓存默认 64 MB，可用 `FUJU_TRACE_ROLLUP_PAGE_CACHE_BYTES` 调整。`bm25.dat` v3 把每个词切成 128 条一块，目录保存块的文档范围和真实最大 BM25 norm；block-max 先判断上界，再决定是否读取块。块快路和整词 WAND 回退共享 `FUJU_TRACE_BM25_POSTINGS_CACHE_BYTES` 预算，整词回退使用连续读。无过滤 BM25 查询结果另有 16 MB 有界 CLOCK 缓存，可用 `FUJU_TRACE_BM25_QUERY_CACHE_BYTES` 调整；索引写入、重建或重新加载后失效，相同冷查询并发进入时只执行一次评分。BM25 支持把结构化过滤下推到打分阶段；候选 postings 集合按 64 MB 临时预算决定是否物化，超过预算才回到小批候选逐条校验。默认 segment 的 `seg-*.idx` 保存 key 到物理字节范围，top-k 回填只读取目标行；候选 key 超过 4096 时回退顺序扫描，避免大量随机读。clean reopen 仍只恢复 manifest、WAL checkpoint 和控制状态；旧版本、缺失、损坏或版本不匹配时按需扫描 segment 重建派生索引。aggregate/trajectory/loop/task 带 `text` 时仍需读取正文并回到正确扫描。
