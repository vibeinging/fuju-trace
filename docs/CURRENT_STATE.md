...
The latest complete PyPI release is 0.1.10 (`fuju-trace`, `fuju-trace-vexdb`, and `fuju-trace-sql`). The 0.1.11 source adds separate SQLite, DuckDB, and PostgreSQL package names, but its PyPI rollout is still in progress; only `fuju-trace-sqlite` 0.1.11 has been uploaded so far. See `docs/reports/2026-09-26_python-0.1.11-release.md` for current publication status and `docs/reports/2026-09-25_sql-adapter-real-tests.md` for SQL test scope. The native `fuju-trace-db` wheel is not part of this PyPI release.
