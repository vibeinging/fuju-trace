...
HOST="${FUJU_TRACE_DEMO_HOST:-127.0.0.1}"
PORT="${FUJU_TRACE_DEMO_PORT:-7878}"
BASE="http://${HOST}:${PORT}"
...
rm -rf "$ROOT/fuju-trace-engine/crates/fuju-trace-engine/console_dist"
cp -R "$ROOT/fuju-trace-console/dist" "$ROOT/fuju-trace-engine/crates/fuju-trace-engine/console_dist"

echo "==> Starting Fuju Trace on ${BASE}"
(
  cd "$ROOT/fuju-trace-engine"
  FUJU_TRACE_BIND="${HOST}:${PORT}" cargo run -p fuju-trace-engine --example server
) &
SERVER_PID=$!
...
if [ "${FUJU_TRACE_DEMO_OPEN:-0}" = "1" ]; then
