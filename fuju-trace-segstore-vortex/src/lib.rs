//! fuju-trace-segstore-vortex —— **列式段存储（Vortex）**，实现引擎的 `SegmentStore` trait。
//!
...
use fuju_trace_engine::{KeyedRecordScan, KeyedSegmentScan, Projection, SegmentStore};
...
impl VortexSegmentStore {
    ...
    fn write_key_index(
        ...
        let crc = fuju_trace_wal::crc32(&bytes);
    ...
    fn read_key_index(&self, seg: SegmentId) -> Option<(Arc<KeyIndex>, u64)> {
        ...
        if fuju_trace_wal::crc32(&bytes[..payload_len]) != stored_crc {
            return None;
        }
        ...
        if data.len() as u64 != data_len || fuju_trace_wal::crc32(&data) != data_crc {
            return None;
        }
        let mut by_key: HashMap<(u64, u64), Vec<u32>> = HashMap::new();
        for row in 0..count {
                ...
                if std::fs::write(&tmp, buf.as_slice()).is_ok() {
                    if std::fs::rename(&tmp, &path).is_ok() {
                        let _ = self.write_key_index(
                            ...
                            records,
                            buf.len() as u64,
                            fuju_trace_wal::crc32(buf.as_slice()),
                        );
                    }
                }
            }
...
mod tests {
    ...
    fn engine_uses_vortex_pushdown_end_to_end() {
        ...
        use fuju_trace_engine::{TraceQuery, WriteCoordinator};
        use std::sync::Arc;
    ...
    fn engine_applies_deletion_vector_to_vortex_point_rows() {
        use fuju_trace_core::ids::WalLsn;
        use fuju_trace_engine::WriteCoordinator;
        use std::sync::Arc;
