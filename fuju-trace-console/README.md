...
Fuju Trace 的控制台前端。**轻但真**的 SPA：组件化 + 虚拟滚动 + 游标分页 + 数据缓存，扛得住上千会话 / 上千 span 的真实量级。构建产物是几个静态文件（~216KB JS / 8KB CSS，gzip 68KB），可用 `rust-embed` 塞进引擎单二进制——运行期仍单机零依赖、气隙可部署。
...
引擎已实现的控制台端点（`fuju-trace-engine/src/http.rs`）：
...
# 2. 拷进引擎，编译期内嵌
cp -r dist ../fuju-trace-engine/crates/fuju-trace-engine/console_dist
# 3. 跑引擎控制台服务（启动自带种子数据）
