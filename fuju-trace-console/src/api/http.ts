...
// VITE_TENANT_ID，或在浏览器 localStorage 写入 fuju.trace.tenantId。
//
...
function tenantId(): string | undefined {
  ...
  return window.localStorage.getItem('fuju.trace.tenantId') ?? window.localStorage.getItem('yitrace.tenantId') ?? undefined
}
...
async function get<T>(path: string): Promise<T> {
  ...
  return parseLosslessJson(await res.text()) as T
}
...
interface RawHit { trace_id: string | number; span_id: string | number; score: number; status: number | null; agent_name: string | null; logs: string[] }
